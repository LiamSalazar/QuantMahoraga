...
║  ─────────────────────────────────────────────────────────────────────────   ║
║  [WALK-FORWARD — fixes #1, #2, #3, #4, #5]                                   ║
║  · Contiguous fold generator: test_start[n] = last market day after          ║
║    test_end[n-1], derived from the REAL trading calendar, not bdate_range    ║
║  · Gap detection against actual trading index, not generic business days     ║
║  · Final config = real winning combo from last fold (no Frankenstein mix)    ║
║  · Statistical support (p, q, label) tied exactly to selected combo          ║
║  · Hyperparameter keys stored explicitly; no fragile string-transform map    ║
║                                                                              ║
║  [STATISTICS — fix #6]                                                        ║
...
║                                                                              ║
║  [UNIVERSE ENGINE — new]                                                      ║
║  · Quarterly reconstitution, point-in-time                                  ║
║  · Eligibility: seasoning ≥ 3m, free-float ≥ 10%, ADDV filter               ║
║  · Ranking: float-adjusted market cap (FFMC), not alpha signals              ║
║  · Buffer rule: top-8 auto, incumbents retained to rank-10, buffer to 13    ║
║  · Snapshots with entry/exit reasons saved per quarter                       ║
║  · Simulation layer for backtest (yfinance proxy, clearly labelled)          ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
║  The canonical universe engine (Section 3B) implements point-in-time GICS   ║
║  classification and float-adjusted market cap ranking as a best-effort       ║
║  approximation. Without CRSP/Compustat PIT access, survivorship bias         ║
║  cannot be fully eliminated. All results are conditional on this universe    ║
║  design and do not constitute evidence free of survivorship bias, nor        ║
║  proof of generalisable alpha. This is a conditional research exercise.      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
───────────────────────────────────────────────────────────────────────────────
  The primary universe consists of mega-cap technology stocks. Without
  CRSP/Compustat PIT access, survivorship bias cannot be fully eliminated.
  Results are conditional on universe design. Not evidence of generalisable
  alpha. This is a conditional research exercise, not a trading recommendation.
═══════════════════════════════════════════════════════════════════════════════
"""
...
class UniverseConfig:
    ...
    Implements quarterly GICS-IT point-in-time reconstitution with
    float-adjusted market cap ranking and incumbency buffers.
    """
...
class Mahoraga13Config:
    ...
    # ── Walk-forward ──────────────────────────────────────────────────────────
    # NOTE: test periods are now derived by the contiguous fold generator.
    # wf_folds defines only (train_end, val_start, val_end) — test_start and
    # test_end are computed automatically to guarantee OOS continuity.
    wf_train_start: str = "2006-01-01"
    wf_folds: Tuple[Tuple[str, str, str], ...] = (
        # (train_end, val_start, val_end)
        ("2013-12-31", "2015-01-01", "2016-12-31"),
        ("2015-12-31", "2017-01-01", "2018-12-31"),
        ("2017-12-31", "2019-01-01", "2020-12-31"),
        ("2019-12-31", "2021-01-01", "2022-12-31"),
        ("2021-12-31", "2023-01-01", "2024-06-30"),
    )
    wf_data_end:    str   = "2026-02-20"   # end of last test window
    ...
    random_seed:    int  = 42
    plots_dir:      str  = "mahoraga13_plots"
    outputs_dir:    str  = "mahoraga13_outputs"
    label:          str  = "MAHORAGA_1.3"
...
def _build_universe_snapshot(
    ...
    Ranking variable: float-adjusted market cap proxy = Price × Volume (30d avg).
    Volume is used as a FFMC proxy since actual shares-outstanding and free-float
    data require CRSP/Compustat. This is clearly labelled as a simulation proxy.

    Returns (selected_members, snapshot_df_with_reasons).
    NOTE: Without CRSP PIT data, survivorship bias cannot be eliminated.
    This function provides the methodological skeleton; plug in real PIT data
    via tickers_in_scope to approach institutional quality.
    """
    idx = close.index
    # Market trading days up to reconstitution date
    past = idx[idx <= date]
    ...
    for t in tickers_in_scope:
        ...
        v_series = volume[t].reindex(past).dropna() if t in volume.columns else pd.Series()

        # Seasoning: must have at least min_seasoning_days of data
        if len(p_series) < universe_cfg.min_seasoning_days:
            rows.append({"ticker": t, "eligible": False, "reason": "seasoning_fail",
                         "ffmc_proxy": 0.0})
            continue

        # Liquidity: ADDV proxy = mean(price × volume) over last 63 days
        if len(v_series) >= 20:
        ...
        if addv < universe_cfg.min_addv_usd:
            rows.append({"ticker": t, "eligible": False, "reason": "liquidity_fail",
                         "ffmc_proxy": addv})
            continue

        # FFMC proxy: price × 30d avg volume (no shares-outstanding without CRSP)
        ...
        ffmc = float((last30_p * last30_v).mean())
        rows.append({"ticker": t, "eligible": True, "reason": "eligible",
                     "ffmc_proxy": ffmc, "addv_proxy": addv})

    snap = pd.DataFrame(rows).sort_values("ffmc_proxy", ascending=False)
    ...
    # Step 1: auto-entry top-8
    top_auto = eligible[eligible["rank"] <= cfg.auto_entry_rank]["ticker"].tolist()
    selected.extend(top_auto)

    # Step 2: retain incumbents within top-10
    for t in eligible[(eligible["rank"] > cfg.auto_entry_rank) &
                       (eligible["rank"] <= cfg.retention_rank)]["ticker"].tolist():
        if t in prior and t not in selected:
            selected.append(t)
            eligible.loc[eligible["ticker"] == t, "reason"] = "retained_incumbent_rank10"

    # Step 3: buffer incumbents to rank-13
    if len(selected) < cfg.target_size:
        for t in eligible[(eligible["rank"] > cfg.retention_rank) &
                           (eligible["rank"] <= cfg.buffer_rank)]["ticker"].tolist():
            if t in prior and t not in selected and len(selected) < cfg.target_size:
                selected.append(t)
                eligible.loc[eligible["ticker"] == t, "reason"] = "retained_incumbent_buffer"

    # Step 4: fill remaining slots by rank
    if len(selected) < cfg.target_size:
        for t in eligible["ticker"].tolist():
            if t not in selected and len(selected) < cfg.target_size:
                selected.append(t)
...
def build_contiguous_folds(
    ...
    """
    FIX (#1, #2): Generate walk-forward folds with a strictly contiguous OOS
    trajectory. test_start[0] = first trading day after val_end[0].
    test_start[n] = first trading day after test_end[n-1].
    Gap detection uses the REAL trading calendar (trading_idx), not bdate_range.

    Each fold dict contains:
      train_start, train_end, val_start, val_end,
      test_start, test_end, is_partial, actual_test_days
    """
    folds = []
    prev_test_end: Optional[pd.Timestamp] = None

    for i, (train_end_s, val_start_s, val_end_s) in enumerate(cfg.wf_folds):
        train_end = pd.Timestamp(train_end_s)
        val_start = pd.Timestamp(val_start_s)
        val_end   = pd.Timestamp(val_end_s)

        # Embargo check
        embargo_end = train_end + pd.offsets.BDay(cfg.embargo_days)
        assert val_start >= embargo_end, (
            f"[fold {i+1}] val_start={val_start_s} violates embargo "
            f"(train_end + {cfg.embargo_days}bd = {embargo_end.date()})"
        )

        # Derive test_start from REAL trading calendar
        if prev_test_end is None:
            # First fold: test starts day after val_end
            future = trading_idx[trading_idx > val_end]
        else:
            # Subsequent folds: test starts day after previous test_end
            future = trading_idx[trading_idx > prev_test_end]

        if len(future) == 0:
            print(f"  [fold {i+1}] No trading days remaining after previous end — stopping")
            break

        test_start = future[0]

        # test_end: for last fold use wf_data_end; for others use next val_end
        if i == len(cfg.wf_folds) - 1:
            test_end_target = pd.Timestamp(cfg.wf_data_end)
        else:
            next_val_end = pd.Timestamp(cfg.wf_folds[i + 1][2])
            test_end_target = next_val_end + pd.offsets.BDay(1)
            # Actually: test_end = day before next fold's val_start
            # We use next train_end as natural boundary, but the generator
            # overrides this via prev_test_end, so we just use available data
            test_end_target = pd.Timestamp(cfg.wf_folds[i][2])  # val_end of THIS fold → next fold starts
            # Better: use one year after test_start as a nominal window
            one_year_out = test_start + pd.DateOffset(years=1)
            avail = trading_idx[(trading_idx >= test_start) & (trading_idx <= one_year_out)]
            test_end_target = avail[-1] if len(avail) else test_start

        # Last fold uses the global data_end
        if i == len(cfg.wf_folds) - 1:
            test_end_target = pd.Timestamp(cfg.wf_data_end)

        avail_test = trading_idx[(trading_idx >= test_start) & (trading_idx <= test_end_target)]
        if len(avail_test) == 0:
            print(f"  [fold {i+1}] No test data available, skipping")
            break
        test_end = avail_test[-1]

        # Partial detection: use REAL trading days, not bdate_range (fix #2)
        ...
        print(f"  [fold {i+1}] train:{cfg.wf_train_start}→{train_end_s}  "
              f"val:{val_start_s}→{val_end_s}  "
              f"test:{test_start.date()}→{test_end.date()} ({actual_td}bd){partial_str}")
...
def _trend(price: pd.Series, cfg: Mahoraga13Config) -> pd.Series:
    votes = []
...
def _mom(price: pd.Series, cfg: Mahoraga13Config) -> pd.Series:
    raw = sum((price / price.shift(w) - 1.0).shift(1) for w in cfg.mom_windows) / len(cfg.mom_windows)
    return (raw.clip(-1.0, 1.0) + 1.0) / 2.0

def _rel(price: pd.Series, bench: pd.Series, cfg: Mahoraga13Config) -> pd.Series:
    raw = sum(
...
def compute_scores(close: pd.DataFrame, qqq: pd.Series, cfg: Mahoraga13Config) -> pd.DataFrame:
    idx  = close.index
...
def vol_target_scale(port_r: pd.Series, cfg: Mahoraga13Config) -> pd.Series:
...
def backtest(
    ...
    """
    Core backtest. universe parameter allows passing a dynamic universe
    (e.g. from canonical engine schedule) instead of cfg.universe_static.
    """
    np.random.seed(cfg.random_seed)
    univ = list(universe) if universe is not None else list(cfg.universe_static)
    # Filter to tickers actually in ohlcv
    univ = [t for t in univ if t in ohlcv["close"].columns]
        ...
        if dt in reb_dates:
            ...
            if not names:
                ...
                if len(lb) < 60:
                    ...
                    ww = hrp_weights(lb)
                    ww = ww[ww.index.isin(names)].reindex(names).fillna(0.0)
                if ww.sum() > 0:
                    ww = ww.clip(upper=cfg.weight_cap) / ww.clip(upper=cfg.weight_cap).sum()
                last_w = pd.Series(0.0, index=univ)
                last_w[names] = ww.reindex(names).fillna(0.0).values
...
def summarize(r, eq, exp, to, cfg: Mahoraga13Config, label="") -> Dict:
...
def asymptotic_sharpe_ci(r: pd.Series, cfg: Mahoraga13Config, alpha: float = 0.05) -> Dict:
...
def alpha_test_nw(r_s, r_b, cfg: Mahoraga13Config, label: str = "",
...
def factor_attribution(r_s, ff, cfg: Mahoraga13Config, label="") -> Optional[Dict]:
...
def regime_analysis(r_s, r_b, ohlcv, cfg: Mahoraga13Config) -> pd.DataFrame:
    idx = to_s(r_s).index
...
def stress_report(r, exp, episodes, cfg: Mahoraga13Config, r_bench=None) -> pd.DataFrame:
    rows = []
...
def baseline_eqw(ohlcv, cfg: Mahoraga13Config, costs: CostsConfig) -> Dict:
    close = ohlcv["close"][list(cfg.universe_static)].copy()
    n = len(cfg.universe_static); idx = close.index
    w    = pd.DataFrame(1.0/n, index=idx, columns=cfg.universe_static)
    rets = close.pct_change().fillna(0.0)
...
def baseline_mom(ohlcv, cfg: Mahoraga13Config, costs: CostsConfig) -> Dict:
    close = ohlcv["close"][list(cfg.universe_static)].copy()
...
def baseline_signal_decomp(ohlcv, cfg: Mahoraga13Config, costs: CostsConfig) -> Dict[str, Dict]:
    results = {}
...
def objective(s_val: Dict, s_qqq: Dict, cfg: Mahoraga13Config) -> float:
...
def run_walk_forward(
    ...
    """
    FIX (#1, #2): Uses build_contiguous_folds() — test_start derived from real
    trading calendar, not bdate_range. OOS continuity validated against real idx.

    FIX (#3): Final config = winning combo of last fold (real observed combo).
    FIX (#4): Statistical support tied exactly to selected combo.
    FIX (#5): Hyperparams stored as explicit combo_params dict; no string mapping.
    ...
    for fold in folds:
        ...
        close_univ = ohlcv["close"][list(cfg_base.universe_static)]
...
def execution_sensitivity_stress(ohlcv, cfg: Mahoraga13Config, base_costs: CostsConfig) -> pd.DataFrame:
    """
...
def stop_keep_cash_ablation(ohlcv, cfg_win: Mahoraga13Config, costs: CostsConfig) -> pd.DataFrame:
    rows = []
...
def ic_weight_ablation(ohlcv, cfg_win: Mahoraga13Config, costs: CostsConfig) -> pd.DataFrame:
    rows = []
...
def plot_risk_overlays(res: Dict, title: str, path: str):
    ...
    for ax, (s, c, lbl) in zip(axes, [
        ...
        ax.fill_between(sv.index, sv.values, alpha=.2, color=c)
...
def plot_signal_decomp(decomp: Dict, bench_r: pd.Series, cfg: Mahoraga13Config, title: str, path: str):
...
def save_outputs(
    ...
    methodology = {
        ...
        "canonical_ranking":  "float_adjusted_market_cap_proxy",
        ...
        "min_free_float":     universe_cfg.min_free_float,
...
def make_plots(out, oos_r, oos_eq, fold_results, ic_df, decomp, rob, cfg,
    ...
    plot_equity({cfg.label: res["equity"], "QQQ": res["bench"]["QQQ_eq"],
                ...
                "Full Period Equity — Mahoraga 1.3", f"{p}/01_equity_full.png")
    plot_equity({cfg.label: oos_eq, "QQQ (OOS)": res["bench"]["QQQ_eq"].reindex(oos_r.index)},
                f"Walk-Forward {oos_label} — Mahoraga 1.3", f"{p}/02_equity_oos.png")
...
def run_mahoraga13(make_plots_flag: bool = True, run_robustness: bool = True) -> Dict:
    print("="*80)
    print("  MAHORAGA 1.3 — Research Edition")
    print("="*80)
    print(UNIVERSE_BIAS_DISCLAIMER)
    ...
    all_tickers = sorted(set(
        list(cfg.universe_static) + [cfg.bench_qqq, cfg.bench_spy, cfg.bench_vix]
    ...
    # ── [1c] Canonical universe engine (simulation proxy) ─────────────────────
    universe_schedule  = None
    universe_snapshots = None
    if cfg.use_canonical_universe:
        print("\n[1c] Building canonical universe (quarterly FFMC ranking) …")
        universe_schedule, universe_snapshots = build_canonical_universe_schedule(
            ohlcv["close"], ohlcv["volume"], ucfg, all_tickers,
            cfg.data_start, cfg.data_end
        )
        print(f"  [universe] {len(universe_schedule)} reconstitution dates built")
        # Use most recent universe as the active static list for this run
        latest_members = get_universe_at_date(universe_schedule, pd.Timestamp(cfg.data_end))
        if latest_members:
            cfg.universe_static = tuple(latest_members)
            print(f"  [universe] Active members: {cfg.universe_static}")
    else:
        print("\n[1c] Using static ex-post universe (survivorship bias present)")
        print(f"  [universe] {cfg.universe_static}")

    # ── [2] Walk-forward (contiguous OOS) ────────────────────────────────────
    print("\n[2] Expanding-window walk-forward (contiguous OOS, real trading calendar) …")
    oos_r, oos_eq, fold_results, all_sweeps, oos_label, selected_config_info = \
        run_walk_forward(ohlcv, cfg, costs)
    ...
    # Re-fit IC on full final train
    close_univ = ohlcv["close"][list(cfg.universe_static)]
...
if __name__ == "__main__":
    results = run_mahoraga13(make_plots_flag=True, run_robustness=True)
