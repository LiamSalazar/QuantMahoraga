...
- columnar;
- compressed;
- generated by `run_extended_analysis.py`;
- read by pandas and pyarrow;
- served through FastAPI;
- consumed by the React DSS frontend.

Parquet is appropriate for this research layer because it is efficient for analytical scans, compact compared with CSV, preserves schema better than raw CSV, is easy to read from Python, and is suitable for reproducible research artifacts. It does not require running a database server, works well at the current cube size, and can later be migrated to DuckDB, SQLite, PostgreSQL, or a warehouse if query volume grows.

```mermaid
flowchart LR
    A["run_extended_analysis.py"] --> B["Parquet audit cubes"]
    B --> C["pandas / pyarrow"]
    C --> D["FastAPI"]
    D --> E["JSON endpoints"]
    E --> F["React DSS frontend"]
```
...
## Logical Audit Schema: Fact Constellation

The physical storage is Parquet, but the analytical design is best understood as a denormalized fact constellation, also known as a galaxy schema.

It is not a pure star schema because there is not one central fact table with separate physical dimension tables. Instead, Mahoraga uses several fact-like Parquet tables that share common analytical dimensions.
...
| Main keys | `date`, `fold`, `candidate_id`, `universe_id` |
| Main measures | `long_budget`, `gate_scale`, `vol_mult`, `exp_cap`, `leader_blend`, `backoff_strength_applied`, `continuation_trigger_p`, `continuation_pressure_p`, `continuation_break_risk_p`, `structural_p`, `hard_backoff_flag` |
| Main dimensions | date, fold, candidate, universe, participation state |
| Research questions | How much capital did Mahoraga deploy? Was risk reduction active? Was continuation supportive? Was structural risk high? |
| Example interpretation | High long budget with bull participation means the model was expressing long-side exposure rather than holding back capital. |
...
| Main keys | `date`, `ticker`, `fold`, `candidate_id`, `universe_id` |
| Main measures | `base_score`, `rank`, `selected_flag`, `base_weight`, `leader_flag`, `final_weight`, `stop_flag`, `pnl_contribution` |
| Main dimensions | date, ticker, fold, candidate, universe, leader state |
| Research questions | Which tickers were selected? Which names had final weight? Which names contributed to PnL? Did stops affect a position? |
| Example interpretation | A selected ticker with positive final weight and positive PnL contribution supported the decision on that date. |
...
| Main keys | `date`, `module_name`, `fold`, `candidate_id`, `universe_id` |
| Main measures | `branch_taken`, `threshold_crossed`, `signal_strength`, `main_inputs_summary_json`, `main_outputs_summary_json` |
| Main dimensions | date, module, fold, candidate, universe |
| Research questions | Which modules influenced the decision? Which branch did each module take? Was a threshold crossed? |
| Example interpretation | A threshold-crossed continuation branch indicates that the continuation layer was active in the audited policy path. |
...
| Main keys | `decision_date`, `horizon`, `fold`, `candidate_id`, `universe_id` |
| Main measures | `realized_return`, `realized_alpha_vs_qqq`, `realized_alpha_vs_spy`, `realized_turnover`, `realized_exposure`, `realized_drawdown_change`, `decision_helped_flag_vs_qqq`, `decision_helped_flag_vs_control`, `continuation_helped_flag`, `backoff_helped_flag`, `leader_helped_flag` |
| Main dimensions | decision date, horizon, fold, candidate, universe |
| Research questions | Did the decision help after 1d, 5d, or 20d? Did it beat QQQ or SPY? Did it beat the control baseline? |
| Example interpretation | Positive 20-day alpha versus QQQ after high participation supports the participation thesis for that case. |
...
| Grain | date |
| Main keys | `date` |
| Main measures | `qqq_return`, `qqq_drawdown`, `qqq_vol`, `spy_return`, `spy_drawdown`, `vix`, `avg_corr`, `breadth`, `market_regime_proxy` |
| Main dimensions | date, market regime |
| Research questions | What market regime surrounded the decision? Was the benchmark in drawdown? Was volatility elevated? Was breadth healthy? |
| Example interpretation | Bull-participation decisions during strong benchmark context can be evaluated separately from decisions taken during fragile regimes. |

## Analytical Operations Supported by the Cubes

| Operation | What it means | Tables used | Question answered | Interpretation |
|---|---|---|---|---|
| Slice | Filter one logical dimension. | Any cube with the dimension. | What happened for one candidate, fold, ticker, module, horizon, or date range? | A slice isolates one analytical context without changing the artifacts. |
| Dice | Filter several dimensions together. | Usually `decision_date_cube`, `position_cube`, `module_trace_cube`, `outcome_cube`. | What happened for official candidate, fold 3, selected NVDA cases, at 20d? | A dice operation narrows the evidence to a specific research case. |
| Drill-down | Move from aggregate metrics to dates, tickers, modules, and outcomes. | `decision_date_cube`, `position_cube`, `module_trace_cube`, `outcome_cube`. | Which concrete decisions explain an aggregate result? | Drill-down prevents stitched performance from hiding case-level behavior. |
| Roll-up | Aggregate granular rows by fold, module, ticker, horizon, or regime. | Any cube, often `position_cube` and `outcome_cube`. | Which folds, modules, or tickers dominate the evidence? | Roll-up summarizes behavior while retaining cube lineage. |
| Decision-to-outcome join | Join a decision state to realized future performance. | `decision_date_cube`, `outcome_cube`. | Did a specific decision help at 1d, 5d, or 20d? | If high participation states are followed by positive 20-day alpha vs QQQ, that supports the participation thesis. If high participation is followed by weak relative outcomes in fragile regimes, risk controls should be reviewed. |
| Module attribution | Group trace rows by module, branch, and threshold state. | `module_trace_cube`, optionally `outcome_cube`. | Which modules were active and how were they associated with outcomes? | Module attribution exposes branch behavior without inventing hidden explanations. |
| Regime audit | Join decision states to benchmark and market context. | `decision_date_cube`, `market_context_cube`, `outcome_cube`. | Did behavior differ in drawdown, high volatility, weak breadth, or bull participation regimes? | Regime audit separates model behavior by market context around the decision. |
| Ticker contribution audit | Aggregate selected positions by ticker and PnL contribution. | `position_cube`. | Which names drove realized contribution? | Ticker contribution links stock selection to outcome evidence. |
| Budget sensitivity audit | Compare official and budget-axis candidates. | `extended_multiplier_summary.csv`, `plateau_radius_by_axis.csv`, `extended_multiplier_fold_summary.csv`. | Does reducing or increasing budget damage robustness? | Budget underdeployment is the main documented local sensitivity in the sampled range. |
| Backoff effectiveness audit | Compare backoff states and helped flags. | `decision_date_cube`, `outcome_cube`, `backoff_audit.csv`. | Did defensive reduction help? | Backoff helped rates are evidence fields, not assumptions that defense always helps. |
| Leader participation audit | Compare leader-active cases and outcome flags. | `position_cube`, `outcome_cube`, `leader_participation_audit.csv`. | Did leader participation help in longer horizons? | Leader states can be evaluated by horizon and by leader ticker contribution. |
| Continuation effectiveness audit | Compare continuation triggers and helped flags. | `decision_date_cube`, `module_trace_cube`, `outcome_cube`, `continuation_activation_audit.csv`. | Did continuation help more at 20d than 1d? | Continuation evidence is strongest where helped rates and alpha improve by horizon. |
| Fold weakness audit | Aggregate outcomes by fold and inspect weak cases. | `outcome_cube`, `extended_multiplier_fold_summary.csv`, `decision_date_cube`. | Which walk-forward folds were weaker and why? | Fold weakness matters because out-of-sample damage can be hidden by a strong aggregate series. |
...
```python
# Hard backoff dates
hard_backoff = decision[decision["hard_backoff_flag"] == True]

# Selected positions with positive final weight
selected = positions[(positions["selected_flag"] == True) & (positions["final_weight"] > 0)]
...
# Average 20-day alpha by participation state
twenty_day = decision_outcome[decision_outcome["horizon"] == 20]
summary = twenty_day.groupby("participation_state")["realized_alpha_vs_qqq"].mean()
```
...
The extended analysis includes a FastAPI app and a React/TypeScript/Tailwind frontend. The frontend is now a DSS-style interpretability layer. It is not a trading interface, not a generic database browser, and not a recomputation engine. It sits on top of FastAPI and the Parquet audit cubes to answer model-research questions with guided presets, robustness summaries, decision investigations, module/outcome audits, and secondary raw cube access.
...
| API app | `research/mahoraga14_3_extended_analysis/api` | Reads materialized CSV/Parquet outputs and returns filtered JSON plus deterministic DSS summaries. |
| Frontend | `research/mahoraga14_3_extended_analysis/frontend` | Research DSS for interpretability, robustness, decision drill-down, and cube inspection. |

The API and frontend are research inspection tools. They do not run live trades, generate broker orders, call external AI services, or replace the underlying CSV/Parquet artifacts.
...
| `GET /outcomes` | Filtered outcome cube records. |
...
| `GET /dss/presets` | Guided decision-investigation preset definitions, counts, and sample cases. |
| `GET /dss/decision-detail` | Decision row plus related positions, module traces, outcomes, market context, and deterministic interpretation strings. |
| `GET /dss/module-effectiveness` | Helped rates, counts, module state frequencies, fold behavior, and leader ticker evidence. |
| `GET /dss/ticker-contribution` | Selected frequency and PnL contribution by ticker. |
| `GET /dss/data-cubes` | Physical file inventory, schemas, logical dimensions, relationships, and sample query shapes. |
