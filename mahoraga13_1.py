...
║  · Eligibility: seasoning ≥ 63d, ADDV ≥ $50M, free-float proxy applied     ║
...
class Mahoraga13Config:
    ...
    #
    # OOS PURITY GUARANTEE (fixes #1 + #5):
    #   - val[n]  and test[m] are disjoint for ALL n, m
    #   - test[n] and test[m] are disjoint for n != m  (exclusive OOS windows)
    #   - test windows are temporally contiguous across folds
    #   - Boundaries are EXPLICIT — no heuristic inference of test_end
    #
    # Cross-fold timeline (no overlap anywhere):
    #   [train₁ | val₁ | test₁] [train₂ | val₂ | test₂] ...
    #   train expands leftward each fold; val and test are strictly exclusive.
    #
    ...
    wf_folds: Tuple[Tuple[str, str, str, str, str], ...] = (
        ...
        ("2019-12-31", "2021-01-04", "2022-12-30", "2023-01-03", "2023-12-29"),
...
def build_contiguous_folds(
    ...
    # Parse all boundaries
    parsed = []
    for entry in cfg.wf_folds:
        parsed.append(tuple(pd.Timestamp(s) for s in entry))

    # Collect intervals for purity audit
    val_days: set  = set()
    test_days: set = set()
    ...
    for i, (te, vs, ve, ts, tend) in enumerate(parsed):
        ...
        # Invariant 5: val before test
        assert ve < ts, (
            f"[fold {fold_n}] val_end={ve.date()} >= test_start={ts.date()}"
        )

        # Invariant 1+2: gather real trading days in each window
        v_days = set(trading_idx[(trading_idx >= vs) & (trading_idx <= ve)].tolist())
        t_days = set(trading_idx[(trading_idx >= ts) & (trading_idx <= tend)].tolist())

        v_t_overlap = v_days & test_days
        assert not v_t_overlap, (
            f"[fold {fold_n}] val {vs.date()}→{ve.date()} overlaps {len(v_t_overlap)} "
            f"days already assigned to a test window. OOS purity violated."
        )

        t_v_overlap = t_days & val_days
        assert not t_v_overlap, (
            f"[fold {fold_n}] test {ts.date()}→{tend.date()} overlaps {len(t_v_overlap)} "
            f"days already assigned to a val window. OOS purity violated."
        )

        t_t_overlap = t_days & test_days
        assert not t_t_overlap, (
            f"[fold {fold_n}] test {ts.date()}→{tend.date()} overlaps {len(t_t_overlap)} "
            f"days already in prior test windows. OOS purity violated."
        )

        val_days  |= v_days
        test_days |= t_days

    # Invariant 3: test windows contiguous on real calendar
    ...
    for i, (te, vs, ve, ts, tend) in enumerate(parsed):
        ...
        print(f"  [fold {i+1}] train:{cfg.wf_train_start}→{te.date()}  "
              f"val:{vs.date()}→{ve.date()}  "
              f"test:{ts.date()}→{tend.date()} ({actual_td}bd){partial_str}")

    print(f"  [wf_purity] val∩test=∅ ✓  test∩test=∅ ✓  contiguous ✓  embargo ✓")
    return folds
...
def run_mahoraga13(make_plots_flag: bool = True, run_robustness: bool = True) -> Dict:
    ...
    if cfg.use_canonical_universe:
        ...
        print("\n[1c] Static ex-post universe (survivorship bias present, use_canonical_universe=False)")
        print(f"  [universe] {list(cfg.universe_static)}")
