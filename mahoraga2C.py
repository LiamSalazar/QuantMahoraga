...
# ─────────────────────────────────────────────────────────────────────────────
# CHANGELOG (1.3 → 1.3.1)
# - Fixed OOS contiguity between folds 4 and 5 by extending fold-4 test_end to 2024-06-28.
# - Kept within-fold purity (val_end < test_start) and OOS exclusivity (test∩test=∅).
# - Maintains the methodological note: validation may reuse prior OOS history in expanding WFO.
# ─────────────────────────────────────────────────────────────────────────────
...
class Mahoraga13Config:
    ...
    wf_folds: Tuple[Tuple[str, str, str, str, str], ...] = (
        ...
        ("2019-12-31", "2021-01-04", "2022-12-30", "2023-01-03", "2024-06-28"),  # extended to ensure contiguous OOS with fold 5
...
def execution_sensitivity_stress(ohlcv, cfg: Mahoraga13Config, base_costs: CostsConfig,
...
def stop_keep_cash_ablation(ohlcv, cfg_win: Mahoraga13Config, costs: CostsConfig,
                             universe_schedule: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    rows = []
...
def ic_weight_ablation(ohlcv, cfg_win: Mahoraga13Config, costs: CostsConfig,
                        universe_schedule: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    rows = []
...
def run_mahoraga13(make_plots_flag: bool = True, run_robustness: bool = True) -> Dict:
    print("="*80)
    print("  MAHORAGA 1.3.1 — Research Edition")
    print("="*80)
    print(UNIVERSE_BIAS_DISCLAIMER)
