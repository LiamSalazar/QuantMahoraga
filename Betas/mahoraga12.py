...
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  Mahoraga 1.2 implements a long-only, weekly-rebalanced tech rotation        ║
...
║  expanding-window walk-forward splits, execution-sensitivity stress tests,   ║
║  local parameter sensitivity maps, alternate-universe evaluation, and        ║
║  multiple-testing-aware reporting.                                            ║
║                                                                              ║
║  Changes from 1.1 → 1.2                                                      ║
║  ─────────────────────────────────────────────────────────────────────────   ║
║  [WALK-FORWARD]                                                               ║
║  · Continuous OOS: test_start[n] = test_end[n-1] + 1 business day           ║
║  · OOS consolidated series is a temporally contiguous trajectory             ║
║  · Partial final fold explicitly labelled as such                            ║
║                                                                              ║
║  [STATISTICS]                                                                 ║
║  · q_value (BHY) now computed and reported per configuration                 ║
║  · Sweep CSV includes: p_value, q_value, significant_5pct, significant_10pct ║
║  · Narrative updated: q_value = FDR support, not certainty                  ║
║                                                                              ║
║  [ROBUSTNESS]                                                                 ║
║  · gap_stress renamed to Execution_Sensitivity_Stress                        ║
║  · Stress described as friction/slippage sensitivity, not gap simulation     ║
║                                                                              ║
║  [METHODOLOGY]                                                                ║
║  · Mandatory universe-bias disclaimer in all reports                         ║
║  · OOS type labelled: stitched / continuous / partial                        ║
║                                                                              ║
║  [ENGINEERING]                                                                ║
║  · Sweep parallelised via joblib (backend=loky, deterministic seeds)         ║
║  · parallel_sweep flag to disable if environment does not support it         ║
║  · Data loaded once; workers receive shared reference                        ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
║  stocks selected ex post based on their prominence and data availability     ║
║  as of the analysis date. All results are conditional on this universe       ║
║  design. They do not constitute evidence free of survivorship bias, nor      ║
║  proof of generalisable alpha beyond this specific universe. The strategy    ║
║  should be interpreted as a conditional research exercise, not as a live     ║
...
# Parallelism — joblib required; graceful fallback if unavailable
try:
    from joblib import Parallel, delayed as joblib_delayed
    _JOBLIB_AVAILABLE = True
except ImportError:
    _JOBLIB_AVAILABLE = False
    print("  [warn] joblib not found — parallel_sweep will be disabled automatically")
...
class CostsConfig:
    ...
    gap_factor  : execution friction multiplier used in Execution_Sensitivity_Stress.
                  NOTE: this does NOT simulate real overnight price gaps.
                  It models degradation of performance under severe execution
                  friction and panic-cost scenarios (operational robustness test).
    qqq_expense : daily fraction of QQQ 20 bp p.a. expense ratio
    """
    commission:       float = 0.0010
    slippage:         float = 0.0003
    apply_slippage:   bool  = True
    gap_factor:       float = 1.0          # 1.0=none; stress values set per scenario
    qqq_expense_ratio:float = 0.0020 / 252


@dataclass
class Mahoraga12Config:
    # ── Universe & benchmarks ─────────────────────────────────────────────────
    universe: Tuple[str, ...] = (
    ...
    # Each fold: train expands, val=2y fixed, test contiguous from previous fold
    # Fold 1: train 2006-2013  val 2014-2015*  test starts day after val_end
    # Fold 2: train 2006-2015  val 2016-2017*  test starts day after fold-1 test_end
    # ... and so on (continuous OOS trajectory)
    ...
    label:          str  = "MAHORAGA_1.2"
    parallel_sweep: bool = True    # Set False if environment does not support joblib/loky
...
# ── Universe bias disclaimer (inserted automatically in all reports) ───────────
...
───────────────────────────────────────────────────────────────────────────────
  ...
  based on their prominence as of the analysis date. All results are
  conditional on this universe design and do not constitute evidence free of
  survivorship bias. Alpha and performance metrics should not be interpreted
  as proof of generalisable alpha beyond this specific universe.
  This is a conditional research exercise, not a live trading recommendation.
═══════════════════════════════════════════════════════════════════════════════
"""

# ── Execution Sensitivity Stress disclaimer ────────────────────────────────────
EXECUTION_STRESS_DISCLAIMER = """
  NOTE — Execution_Sensitivity_Stress: These scenarios evaluate performance
  degradation under severe execution friction (slippage sensitivity, cost
  sensitivity, panic-execution penalties). They do NOT simulate realistic
  overnight price gaps or actual market microstructure events. They are
  simplified operational robustness tests only.
"""
...
def hrp_weights(returns: pd.DataFrame) -> pd.Series:
    ...
    # FIX: guard against NaN/inf in correlation before distance matrix
    corr_clean = corr.fillna(0.0)
    np.fill_diagonal(corr_clean.values, 1.0)
    dm = np.sqrt(np.clip(0.5 * (1.0 - corr_clean.values), 0.0, 1.0))
    np.fill_diagonal(dm, 0.0)

    # Ensure condensed distance matrix is fully finite before linkage
    dm_condensed = squareform(dm, checks=False)
    if not np.all(np.isfinite(dm_condensed)):
        # Fallback: replace any remaining non-finite with max observed distance
        dm_condensed = np.where(np.isfinite(dm_condensed), dm_condensed,
                                np.nanmax(dm_condensed[np.isfinite(dm_condensed)], initial=1.0))

    lnk = linkage(dm_condensed, method="single")
    ...
    ordered = corr_clean.index[_qd(lnk)]
...
def _trend(price: pd.Series, cfg: Mahoraga12Config) -> pd.Series:
    votes=[]
...
def _mom(price: pd.Series, cfg: Mahoraga12Config) -> pd.Series:
    raw = sum((price/price.shift(w)-1.0).shift(1) for w in cfg.mom_windows)/len(cfg.mom_windows)
    return ((raw.clip(-1.0,1.0)+1.0)/2.0)

def _rel(price: pd.Series, bench: pd.Series, cfg: Mahoraga12Config) -> pd.Series:
    raw = sum(
...
def compute_scores(close: pd.DataFrame, qqq: pd.Series, cfg: Mahoraga12Config) -> pd.DataFrame:
    idx  = close.index
...
def vol_target_scale(port_r: pd.Series, cfg: Mahoraga12Config) -> pd.Series:
...
def summarize(r, eq, exp, to, cfg: Mahoraga12Config, label="") -> Dict:
...
def asymptotic_sharpe_ci(r: pd.Series, cfg: Mahoraga12Config, alpha: float = 0.05) -> Dict:
...
def alpha_test_nw(r_s, r_b, cfg: Mahoraga12Config, label: str = "",
...
def factor_attribution(r_s, ff, cfg: Mahoraga12Config, label="") -> Optional[Dict]:
...
def regime_analysis(r_s, r_b, ohlcv, cfg: Mahoraga12Config) -> pd.DataFrame:
    idx = to_s(r_s).index
    vix_available = False
...
def stress_report(r, exp, episodes, cfg: Mahoraga12Config, r_bench=None) -> pd.DataFrame:
    rows=[]
...
def baseline_eqw(ohlcv, cfg: Mahoraga12Config, costs: CostsConfig) -> Dict:
...
def baseline_mom(ohlcv, cfg: Mahoraga12Config, costs: CostsConfig) -> Dict:
    close = ohlcv["close"][list(cfg.universe)].copy()
...
def baseline_signal_decomp(ohlcv, cfg: Mahoraga12Config, costs: CostsConfig) -> Dict[str,Dict]:
    results={}
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 12 — SWEEP & OBJECTIVE  (with BHY q-values and parallelisation)
# ═══════════════════════════════════════════════════════════════════════════════

def objective(s_val: Dict, s_qqq: Dict, cfg: Mahoraga12Config) -> float:
...
def bhy_q_values(p_values: np.ndarray, alpha: float = 0.05) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Applies the c(m) = sum(1/k) correction factor for arbitrary dependence.
    Interpretation: q_value is the minimum FDR level at which the hypothesis
    would be rejected. A low q_value provides statistical support under FDR
    control — it does NOT imply certainty or absence of data-mining bias.
    """
    m    = len(p_values)
    c_m  = np.sum(1.0 / np.arange(1, m + 1))   # harmonic correction
    order   = np.argsort(p_values)
    p_sorted = p_values[order]
    ranks    = np.arange(1, m + 1)

    # q_i = p_i * (m * c_m) / rank_i
    q_raw = p_sorted * (m * c_m) / ranks

    # Monotonise: q[i] = min(q[i], q[i+1], ..., q[m-1])
    q_mono = np.minimum.accumulate(q_raw[::-1])[::-1]
    q_clamped = np.clip(q_mono, 0.0, 1.0)

    # Map back to original order
    q_result = np.empty(m)
    q_result[order] = q_clamped
...
def bhy(p_values: np.ndarray, alpha: float = 0.05) -> np.ndarray:
    """Legacy helper — returns significance flags only (wraps bhy_q_values)."""
    _, sig = bhy_q_values(p_values, alpha)
    return sig
...
def _evaluate_single_combo(
    ...
    """
    Worker function for a single sweep combination.
    Deterministic seed = base_seed XOR hash(combo) to guarantee
    reproducibility regardless of execution order.
    """
    ...
    combo_bytes = json.dumps(kw, sort_keys=True).encode()
    combo_int   = int(hashlib.md5(combo_bytes).hexdigest(), 16) % (2**31)
    ...
    try:
        ...
        row = {
            **kw, "fold": fold_n, "score_val": sc,
            ...
            # q_value and significance flags filled after all combos are done
            "q_value":       np.nan,
            "significant_5pct":  False,
            "significant_10pct": False,
            "_cfg":          cfg,   # kept temporarily for best-config selection
            "_s_val":        s_v,
            "_s_qqq":        s_q,
        }
        return row
    except Exception as e:
        print(f"  [sweep worker] fold={fold_n} combo={kw} ERROR: {e}")
        return None
...
def run_fold_sweep(
    ...
    """
    Run SWEEP_GRID on a single fold's val period.
    Parallelised via joblib (loky backend) when cfg_base.parallel_sweep=True
    and joblib is available. Each worker gets a deterministic seed derived
    from the base seed and combo hash — results are identical regardless of
    parallel/serial execution.

    Memory note: ohlcv (~500 MB) is passed as a function argument.
    Under loky on Linux, fork-based copy-on-write keeps actual RSS low.
    On Windows/macOS, each worker pickles the dict — expect higher RAM usage.
    Disable parallel_sweep=True if this causes issues in your environment.
    """
    ...
    use_parallel = (
        cfg_base.parallel_sweep
        and _JOBLIB_AVAILABLE
        and len(combos) > 8
    )

    if use_parallel:
        print(f"  [sweep] parallel mode (joblib/loky), {len(combos)} combos …")
        raw_rows = Parallel(n_jobs=-1, backend="loky", verbose=0)(
            joblib_delayed(_evaluate_single_combo)(
                combo, keys, cfg_base, ic_weights, ohlcv, costs,
                val_start, val_end, fold_n, base_seed
            )
            for combo in combos
        )
    else:
        if cfg_base.parallel_sweep and not _JOBLIB_AVAILABLE:
            print("  [sweep] joblib unavailable — running serially")
        else:
            print(f"  [sweep] serial mode, {len(combos)} combos …")
        raw_rows = [
    ...
    if not rows:
        raise RuntimeError(f"[fold {fold_n}] All sweep combos failed — check data availability.")

    # ── Compute q-values over the FULL family of evaluated configurations ──
    p_vals = np.array([r["p_value"] for r in rows])
    q_vals, sig_5  = bhy_q_values(p_vals, alpha=0.05)
    _, sig_10       = bhy_q_values(p_vals, alpha=0.10)

    for i, row in enumerate(rows):
        row["q_value"]          = round(float(q_vals[i]), 6)
        row["significant_5pct"] = bool(sig_5[i])
        row["significant_10pct"]= bool(sig_10[i])

    # HLZ t-min threshold (for reference)
    c_n   = np.sum(1.0 / np.arange(1, len(combos) + 1))
    t_min = stats.norm.ppf(1.0 - (0.05 / (len(combos) * c_n)) / 2.0)

    # Build final rows (drop internal keys)
    final_rows = []
    for row in rows:
        cfg_  = row.pop("_cfg")
        s_val = row.pop("_s_val")
        s_qqq = row.pop("_s_qqq")
        row["SR_above_HLZ_tmin"] = row["VAL_SR_tstat"] > t_min
        # Stat label — clear language, no "absolute confidence" framing
        if row["significant_5pct"]:
            row["stat_label"] = "sig@5%_FDR_support"
        elif row["significant_10pct"]:
            row["stat_label"] = "sig@10%_FDR_support"
        else:
            row["stat_label"] = "econ_strong_stat_weak"
        row["_cfg_ref"]  = cfg_
        row["_s_val_ref"]= s_val
        row["_s_qqq_ref"]= s_qqq
        final_rows.append(row)

    df = pd.DataFrame(final_rows).sort_values("score_val", ascending=False)

    # Best config — highest objective score
    best_row = df.iloc[0]
    best_cfg  = best_row["_cfg_ref"]
    best_s_v  = best_row["_s_val_ref"]
    best_s_q  = best_row["_s_qqq_ref"]
    best = {"score": best_row["score_val"], "cfg": best_cfg,
            "s_val": best_s_v, "s_qqq": best_s_q}

    # Drop internal ref columns before returning CSV-ready DataFrame
    df = df.drop(columns=["_cfg_ref","_s_val_ref","_s_qqq_ref"])

    return df, best


# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 13 — WALK-FORWARD ENGINE  (continuous OOS trajectory)
# ═══════════════════════════════════════════════════════════════════════════════
...
def run_walk_forward(
    ...
    ── Continuous OOS (1.2 change) ──────────────────────────────────────────
    test_start[fold 1] = val_end[fold 1] + 1 business day  (unchanged)
    test_start[fold n] = test_end[fold n-1] + 1 business day  (NEW)

    This guarantees the stitched OOS curve is a temporally contiguous
    trajectory with no gaps between folds. The final fold is flagged as
    partial if its length differs from the nominal test size.

    Returns:
      - oos_r:        continuous OOS daily returns
      - oos_eq:       cumulative equity of continuous OOS trajectory
      - fold_results: per-fold metric dicts
      - all_sweeps:   combined sweep DataFrame (includes q_value column)
    """
    ...
    # Track the end of the previous fold's test window to enforce continuity
    prev_test_end: Optional[pd.Timestamp] = None

    for fold_n, (train_end, val_start, val_end, nominal_test_end) in enumerate(cfg_base.wf_folds):
        print(f"\n  ── FOLD {fold_n+1}/5 ──")

        # Validate embargo & derive val-based test_start
        val_derived_test_start = validate_fold(
            train_start, train_end, val_start, val_end,
            nominal_test_end, cfg_base.embargo_days, fold_n+1
        )

        # ── Continuous OOS: override test_start after fold 1 ──────────────
        if prev_test_end is not None:
            cont_test_start = prev_test_end + pd.offsets.BDay(1)
            test_start = str(cont_test_start.date())
            if pd.Timestamp(test_start) > pd.Timestamp(val_derived_test_start):
                # Continuity pushes start further — use continuous start
                print(f"  [fold {fold_n+1}] OOS continuity: test_start adjusted "
                      f"{val_derived_test_start} → {test_start}")
            else:
                test_start = val_derived_test_start
        else:
            test_start = val_derived_test_start

        test_end = nominal_test_end

        # ── Detect partial fold ────────────────────────────────────────────
        # A fold is partial if it is shorter than the reference test size (252bd)
        nominal_td = cfg_base.trading_days
        actual_bd  = len(pd.bdate_range(test_start, test_end))
        is_partial = actual_bd < int(nominal_td * 0.90)   # <90% of nominal = partial
        if is_partial:
            print(f"  [fold {fold_n+1}] PARTIAL FOLD: {actual_bd}bd < {nominal_td}bd threshold")
        ...
        s_test   = summarize(r_test, eq_test, exp_test,
                             ...
                             best_cfg, f"FOLD{fold_n+1}_TEST")

        qqq_test      = res_test["bench"]["QQQ_r"].loc[test_start:test_end]
        s_qqq         = summarize(qqq_test,
                                  best_cfg.capital_initial*(1.0+qqq_test).cumprod(),
                                  None, None, best_cfg, "QQQ")
        alpha_nw_fold = alpha_test_nw(r_test, qqq_test, best_cfg, f"fold{fold_n+1}_test")

        # Best sweep row — for reporting p/q values alongside selection
        top_row  = sweep_df.iloc[0]
        top_pval = float(top_row.get("p_value", np.nan))
        top_qval = float(top_row.get("q_value", np.nan))
        top_sig5 = bool(top_row.get("significant_5pct", False))
        ...
        fold_results.append({
            ...
            "val_p_value":       round(top_pval, 6),
            "val_q_value":       round(top_qval, 6),
            "val_sig_5pct":      top_sig5,
        ...
        # Advance the continuity pointer
        prev_test_end = pd.Timestamp(test_end)

        print(f"  [fold {fold_n+1}] test Sharpe={s_test['Sharpe']:.3f}  "
              f"CAGR={s_test['CAGR']*100:.1f}%  MaxDD={s_test['MaxDD']*100:.1f}%  "
              f"α_ann={alpha_nw_fold.get('alpha_ann',0)*100:.1f}% "
              f"(t={alpha_nw_fold.get('t_alpha',0):.2f})  "
              f"q={top_qval:.4f}  {'PARTIAL' if is_partial else ''}")

    # ── Stitch continuous OOS ──────────────────────────────────────────────
    oos_r = pd.concat(all_oos_r).sort_index()
    oos_r = oos_r[~oos_r.index.duplicated()]

    # Verify continuity — flag any gaps
    has_gaps = False
    oos_bd   = pd.bdate_range(oos_r.index[0], oos_r.index[-1])
    missing  = oos_bd.difference(oos_r.index)
    if len(missing) > 5:   # allow small calendar gaps
        has_gaps = True
        print(f"  [wf] WARNING: {len(missing)} business days missing in OOS — "
              f"curve is stitched but NOT fully continuous")

    oos_label = "OOS_continuous" if not has_gaps else "OOS_stitched_with_gaps"
    oos_eq    = cfg_base.capital_initial * (1.0 + oos_r).cumprod()

    print(f"\n  [wf] OOS type: {oos_label}")
    print(f"  [wf] OOS period: {oos_r.index[0].date()} → {oos_r.index[-1].date()}")

    return oos_r, oos_eq, fold_results, pd.concat(all_sweeps, ignore_index=True), oos_label
...
def execution_sensitivity_stress(ohlcv, cfg: Mahoraga12Config, base_costs: CostsConfig) -> pd.DataFrame:
    """
    Execution_Sensitivity_Stress (formerly gap_stress in 1.1).

    Evaluates performance degradation under increasingly severe execution
    friction scenarios. Scenarios model:
      - Cost sensitivity (×2, ×3 transaction costs)
      - Slippage sensitivity under panic execution
      - Heavy execution friction (combined cost and slippage penalties)

    IMPORTANT: These are NOT simulations of realistic overnight price gaps
    or actual market microstructure events. They are simplified operational
    robustness tests to assess how fragile strategy returns are to execution
    assumptions.
    """
    scenarios = {
        "BASE":                  CostsConfig(commission=base_costs.commission,
                                             slippage=base_costs.slippage),
        "COST_SENSITIVITY_×2":   CostsConfig(commission=base_costs.commission*2,
                                             slippage=base_costs.slippage*2),
        "COST_SENSITIVITY_×3":   CostsConfig(commission=base_costs.commission*3,
                                             slippage=base_costs.slippage*3),
        "SLIPPAGE_SENSITIVITY":  CostsConfig(commission=base_costs.commission,
                                             slippage=base_costs.slippage,
                                             gap_factor=1.5),
        "PANIC_EXECUTION_HEAVY": CostsConfig(commission=base_costs.commission,
                                             slippage=base_costs.slippage,
                                             gap_factor=2.5),
    }
    rows=[]
    for name, c in scenarios.items():
        ...
        rows.append({
            ...
            "TurnAnn":      round(s["TurnoverAnn"], 2),
            "FinalEq":      round(s["FinalEquity"], 0),
        })
        print(f"  [exec_stress] {name:<28} Sharpe={s['Sharpe']:.3f}  "
              f"CAGR={s['CAGR']*100:.1f}%  DD={s['MaxDD']*100:.1f}%")
    return pd.DataFrame(rows)
...
def ic_weight_ablation(ohlcv, cfg_win: Mahoraga12Config, costs: CostsConfig) -> pd.DataFrame:
    rows=[]
...
def plot_wf_oos(oos_eq: pd.Series, qqq_eq: pd.Series,
    ...
    """
    BUG FIX (1.1 → 1.2):
    fold_labels were pd.Timestamp objects, causing matplotlib to enter a
    circular unit-conversion loop (category → date → TypeError on tz).
    Fix: always convert fold labels to plain Python strings before plotting.
    """
    fig, axes = plt.subplots(2, 1, figsize=(14, 10), sharex=False)  # no sharex for bar subplot
    ...
    # ── BUG FIX: force fold_labels to plain strings ──────────────────────
    ax2 = axes[1]
    fold_sharpes = [float(f["test_Sharpe"]) for f in fold_results]
    fold_labels  = [f"F{int(f['fold'])}" for f in fold_results]   # plain str, no Timestamps
    ...
    # Annotate partial folds
    for i, fd in enumerate(fold_results):
        if fd.get("is_partial"):
            ax2.text(i, fold_sharpes[i] + 0.05, "PARTIAL", ha="center",
...
def plot_signal_decomp(decomp: Dict, bench_r: pd.Series, cfg: Mahoraga12Config,
                       title: str, path: str):
...
def final_evaluation(
    ...
    s_oos      = summarize(oos_r, oos_eq, None, None, cfg_win, oos_label)
...
def print_results(out, fold_results, ff):
    ...
    print(f"\n{sep}\n  MAHORAGA 1.2 — FULL RESULTS\n{sep}")
    oos_label = out.get("oos_label", "OOS_stitched")
...
def save_csvs(out, fold_results, ic_df, rob, out_dir):
    ...
    if rob.get("exec_stress")    is not None: rob["exec_stress"].to_csv(f"{out_dir}/execution_sensitivity_stress.csv",index=False)
    ...
    pd.DataFrame([out["sr_ci_full"],out["sr_ci_oos"]]).to_csv(f"{out_dir}/sharpe_ci.csv",index=False)
...
def make_plots(out, oos_r, oos_eq, fold_results, ic_df, decomp, rob, cfg,
    ...
    plot_equity({cfg.label:res["equity"],"QQQ":res["bench"]["QQQ_eq"],
                ...
                "Full Period Equity — Mahoraga 1.2",f"{p}/01_equity_full.png")
    plot_equity({cfg.label:oos_eq,"QQQ (OOS)":res["bench"]["QQQ_eq"].reindex(oos_r.index)},
                f"Walk-Forward {oos_label} Equity — Mahoraga 1.2",f"{p}/02_equity_oos.png")
    ...
    plot_wf_oos(oos_eq, res["bench"]["QQQ_eq"].reindex(oos_r.index),
                fold_results,f"Walk-Forward OOS by Fold — {oos_label}",
                f"{p}/04_walkforward.png", oos_label=oos_label)
...
def run_mahoraga12(make_plots_flag: bool = True, run_robustness: bool = True) -> Dict:
    print("="*80)
    print("  MAHORAGA 1.2 — Research Edition")
    print("="*80)
    print(UNIVERSE_BIAS_DISCLAIMER)
    ...
    # ── Walk-forward (continuous OOS) ─────────────────────────────────────────
    print("\n[2] Expanding-window walk-forward (5 folds, continuous OOS) …")
    oos_r, oos_eq, fold_results, all_sweeps, oos_label = run_walk_forward(ohlcv, cfg, costs)
    ...
    # Choose final config: use last fold's best
    ...
    wt, wm, wr    = fit_ic_weights(close_univ, qqq_tr_final, cfg_final,
                                   cfg.wf_train_start, last_train_end)
    cfg_final.w_trend = wt; cfg_final.w_mom = wm; cfg_final.w_rel = wr

    # Modal hyperparams from fold sweeps
    ...
    # Report selected config with p/q values from last fold's best
    ...
    print(f"  Selected config stats: p_value={last_fold['val_p_value']:.5f}  "
          f"q_value={last_fold['val_q_value']:.5f}  "
          f"sig@5%={last_fold['val_sig_5pct']}  "
          f"stat_label={last_fold['val_stat_label']}")
    if not last_fold['val_sig_5pct']:
        print("  [NOTE] Best config does NOT pass BHY@5% — strong economically but "
              "statistical support is weak under FDR control. Interpret with caution.")
    ...
    if run_robustness:
        ...
        print("\n  IC ABLATION:")
        print(rob["ic_ablation"].to_string(index=False))
    ...
    # ── Save sweeps (now includes q_value column) ─────────────────────────────
    ...
    print(f"  Sweep CSV columns include: p_value, q_value, significant_5pct, significant_10pct")
...
if __name__ == "__main__":
    results = run_mahoraga12(make_plots_flag=True, run_robustness=True)
