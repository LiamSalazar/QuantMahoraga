  ...
  return (
    <div className="view-grid">
      ...
      <ChartPanel title="Source Relation Usage" question="Which facts or marts support active DSS views?" source="oltp.dss_query_log" ready={sourceRows.length >= 2}>
        <ResponsiveContainer width="100%" height="100%"><BarChart data={sourceRows.slice(0, 24)} layout="vertical"><CartesianGrid stroke="#22303a" /><XAxis type="number" label={axisLabel("Query count")} /><YAxis dataKey="source_relation" type="category" width={190} label={axisLabel("Source relation", true)} /><Tooltip content={<ChartTooltip />} /><Bar dataKey="query_count" fill="#72f0b1" /></BarChart></ResponsiveContainer>
      </ChartPanel>
      ...
      <section className="panel span-6"><SectionHeader title="Query Performance Table" question="Endpoint, source relation, materialized-view use, latency and rows returned." source="/data/execution-evidence" /><DataTable rows={rows} columns={["endpoint", "source_relation", "used_materialized_view", "query_count", "avg_elapsed_ms", "p95_elapsed_ms", "avg_rows_returned", "last_seen_at"]} /></section>
    </div>
  );
}
