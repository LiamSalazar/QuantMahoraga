...
from conviction_amplifier_layer import apply_conviction_amplifier_layer
from continuation_v2_model import apply_continuation_v2_model, fit_continuation_v2_model
from leader_participation_layer import (
    apply_leader_participation_layer,
    derive_leader_participation_engine_params,
)
...
from participation_allocator_v2 import apply_participation_allocator_v2, fit_participation_allocator_v2
from risk_backoff_layer_v2 import apply_risk_backoff_layer_v2
...
def _run_primary_participation_backtest(
    ...
    allocator_weekly_raw = apply_participation_allocator_v2(allocator_fit, weekly_full.loc[:test_end], cfg_fold)
    allocator_weekly_amp = apply_conviction_amplifier_layer(allocator_weekly_raw, weekly_full.loc[:test_end], cfg_fold)
    allocator_weekly = apply_risk_backoff_layer_v2(allocator_weekly_amp, weekly_full.loc[:test_end], cfg_fold)
    ...
    allocator_daily = _weekly_to_daily_state(
        ...
        numeric_defaults={
            ...
            "leader_blend": 0.0,
            "conviction_multiplier_raw": 1.0,
            "conviction_multiplier": 1.0,
            "leader_multiplier_raw": 1.0,
            "leader_multiplier": 1.0,
            "cash_budget_target_raw": float(cfg_fold.participation_long_budget_base),
            "cash_budget_target": float(cfg_fold.participation_long_budget_base),
            ...
            "volatility_health_score": 0.0,
            "fragility_score": 0.0,
            "benchmark_weakness_score": 0.0,
            "cash_drag_pressure_score": 0.0,
            "leader_opportunity_score": 0.0,
            "residual_relief_score": 0.0,
            "bull_score_raw": 0.0,
            "backoff_score_raw": 0.0,
            "participation_pressure_score": 0.0,
            "underparticipation_score": 0.0,
            "conviction_regime_score": 0.0,
            "conviction_amplifier_score": 0.0,
            "conviction_activation": 0.0,
        },
        text_defaults={
    ...
    if pre_test_mask.any():
        ...
        allocator_daily.loc[pre_test_mask, "conviction_multiplier"] = 1.0
        allocator_daily.loc[pre_test_mask, "leader_multiplier"] = 1.0
        allocator_daily.loc[pre_test_mask, "cash_budget_target"] = float(cfg_fold.participation_long_budget_base)
    ...
    primary_weights_1x, leader_diag = apply_leader_participation_layer(
    ...
    return {
        ...
        "override_weekly": override_weekly.assign(fold=fold_n),
        "override_daily": override_daily,
        "allocator_weekly_raw": allocator_weekly_raw,
        "allocator_weekly_amplified": allocator_weekly_amp,
        "allocator_weekly": allocator_weekly,
        "allocator_daily": allocator_daily,
        "bull_diagnostics": leader_diag,
        "leader_diagnostics": leader_diag,
...
def _run_single_fold(
    ...
    base_cache_cal = engine_cache_cal[base_key]
    participation_engine_params = derive_leader_participation_engine_params(best, cfg_fold)
    participation_key = (
    ...
    allocator_fit = fit_participation_allocator_v2(weekly_cal.loc[:train_end].copy(), cfg_fold)
    ...
    fold_row = {
        ...
        "PrimaryAvgConviction": round(float(variant_runs[cfg_fold.combo_variant_key]["allocator_daily"]["conviction_multiplier"].loc[test_start:test_end].mean()), 4),
        ...
        "PrimaryLeaderActiveWeight": round(float(variant_runs[cfg_fold.combo_variant_key]["bull_diagnostics"]["leader_active_weight"].loc[test_start:test_end].mean()), 4),
    ...
    selected_candidate = {
        ...
        "ParticipationBudgetBase": cfg_fold.participation_long_budget_base,
        "ParticipationBudgetCeiling": cfg_fold.participation_long_budget_ceiling,
        "ParticipationLeaderBlendMax": cfg_fold.participation_leader_blend_max,
        "ConvictionMaxWeightScale": cfg_fold.conviction_weight_scale_max,
