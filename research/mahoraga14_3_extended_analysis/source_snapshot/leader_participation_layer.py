...
def apply_leader_participation_layer(
    ...
    for dt in idx:
        ...
        signal_top = leader_signal.nlargest(min(top_k, len(leader_signal)))
        ...
        if top_names and blend > 0.0:
            extra_strength = float(cfg.leader_tilt_strength_max) * blend * max(0.0, float(leader_multiplier.loc[dt]) - 1.0 + 0.25)
