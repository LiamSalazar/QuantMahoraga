...
def _label_map(cfg: Mahoraga14Config) -> Dict[str, str]:
    return {
        ...
        cfg.primary_variant_key: "MAHORAGA14_3_LONG_PARTICIPATION",
    }
        ...
        rows.append(
            {
                ...
                "Start": start,
                "End": end,
                "Mahoraga14_3Return": p_qqq["Return"],
...
def _build_active_return_vs_qqq(wf: Dict[str, Any], cfg: Mahoraga14Config) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(allocator_daily):
        ...
        df["PrimaryConviction"] = allocator_daily["conviction_multiplier"].reindex(df.index).ffill().fillna(1.0)
        df["PrimaryLeaderMultiplier"] = allocator_daily["leader_multiplier"].reindex(df.index).ffill().fillna(1.0)
        df["PrimaryCashBudgetTarget"] = allocator_daily["cash_budget_target"].reindex(df.index).ffill().fillna(cfg.participation_long_budget_base)
...
def _estimate_window_decomp(
    ...
    leader_underweight_est = (
        -(
            bull["leader_top_share_before"].reindex(idx).fillna(0.0)
            - bull["leader_active_weight"].reindex(idx).fillna(0.0)
        ).clip(lower=0.0)
        * qqq_r.clip(lower=0.0)
    ).sum()
    selection_gap_est = ((final_ret - base_def_ret) - alloc["leader_blend"].reindex(idx).fillna(0.0) * qqq_r).sum()
    ...
    return {
        ...
        "EstimatedNameSelectionContribution": float(selection_gap_est),
        "EstimatedLeaderUnderweightContribution": float(leader_underweight_est),
        ...
        "AvgConvictionMultiplier": float(alloc["conviction_multiplier"].mean()),
        "AvgLeaderMultiplier": float(alloc["leader_multiplier"].mean()),
        ...
        "AvgLeaderActiveWeight": float(bull["leader_active_weight"].mean()),
        "AvgLeaderTopShareBefore": float(bull["leader_top_share_before"].mean()),
    }
            ...
            rows.append(
                {
                    ...
                    "AvgWeight_Mahoraga14_3": float(primary_w.get(ticker, 0.0)),
                    ...
                    "SelectionRate_Mahoraga14_3": float((primary_panel.loc[ticker] > 0.0).mean()) if ticker in primary_panel.index else 0.0,
                    ...
                    "CapturedBy14_3": int(primary_w.get(ticker, 0.0) > 1e-6),
                }
            )
    return pd.DataFrame(rows).round(8)
...
def _curve_plot(curve_df: pd.DataFrame, out_png: Path) -> None:
    ...
    plt.plot(curve_df["Date"], curve_df["CumPrimary"], label="Mahoraga14_3", linewidth=1.2)
    plt.plot(curve_df["Date"], curve_df["CumQQQ"], label="QQQ", linewidth=1.2)
    plt.axhline(0.0, color="black", linewidth=0.8, alpha=0.5)
    plt.legend()
    plt.title("Mahoraga14_3 Active Return vs QQQ")
    plt.tight_layout()
...
def _stress_row(
    ...
    return {
        "Variant": "MAHORAGA14_3_LONG_PARTICIPATION",
        "Scenario": scenario,
        "ScenarioNote": note,
...
def _build_stress_and_robustness(wf: Dict[str, Any], cfg: Mahoraga14Config, costs: m6.CostsConfig) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    conviction = allocator.get("conviction_multiplier", pd.Series(1.0, index=primary_r.index)).reindex(primary_r.index).fillna(1.0)
    leader_mult = allocator.get("leader_multiplier", pd.Series(1.0, index=primary_r.index)).reindex(primary_r.index).fillna(1.0)
    leader_diag = _stitch_variant_frame(wf["results"], cfg.primary_variant_key, "bull_diagnostics").reindex(primary_r.index).ffill()
    leader_active = leader_diag.get("leader_active_weight", pd.Series(0.0, index=primary_r.index)).reindex(primary_r.index).fillna(0.0)
    ...
    scenarios = {
        ...
        "CONVICTION_WEAKER": _object_from_returns(primary_r - 0.18 * (conviction - 1.0).clip(lower=0.0) * primary_r.clip(lower=0.0), exposure, turnover, tc, cfg),
        "CONVICTION_STRONGER": _object_from_returns(primary_r + 0.18 * (conviction - 1.0).clip(lower=0.0) * primary_r.clip(lower=0.0), exposure, turnover, tc, cfg),
        "LEADER_PARTICIPATION_WEAKER": _object_from_returns(primary_r - 0.18 * leader * primary_r.clip(lower=0.0) * (qqq_r > 0.0).astype(float), exposure, turnover, tc, cfg),
        "LEADER_PARTICIPATION_STRONGER": _object_from_returns(primary_r + 0.18 * leader * primary_r.clip(lower=0.0) * (qqq_r > 0.0).astype(float) * leader_mult.clip(lower=1.0), exposure, turnover, tc, cfg),
    ...
    notes = {
        ...
        "CONVICTION_WEAKER": "conviction amplification weaker proxy",
        "CONVICTION_STRONGER": "conviction amplification stronger proxy",
        "LEADER_PARTICIPATION_WEAKER": "leader participation weaker proxy",
        "LEADER_PARTICIPATION_STRONGER": "leader participation stronger proxy",
    ...
    for sample_id in range(int(cfg.diagnostics_bootstrap_samples)):
        ...
        robust_rows.append({"Variant": "MAHORAGA14_3_LONG_PARTICIPATION", "Method": "stationary_block_bootstrap", "SampleId": sample_id, "CAGR": summary["CAGR"], "Sharpe": summary["Sharpe"], "MaxDD": summary["MaxDD"]})
    ...
    for budget_mult in (cfg.diagnostics_allocator_tighter_mult, 1.0, cfg.diagnostics_allocator_looser_mult):
        for conviction_mult in (cfg.diagnostics_conviction_weaker_mult, 1.0, cfg.diagnostics_conviction_stronger_mult):
            for leader_part_mult in (cfg.diagnostics_leader_weaker_mult, 1.0, cfg.diagnostics_leader_stronger_mult):
                scaled = primary_r * np.clip(np.minimum(1.0, budget * budget_mult) / budget.replace(0.0, np.nan), 0.75, 1.20).fillna(1.0)
                scaled = scaled + (conviction_mult - 1.0) * (conviction - 1.0).clip(lower=0.0) * scaled.clip(lower=0.0)
                scaled = scaled + (leader_part_mult - 1.0) * leader * scaled.clip(lower=0.0) * (qqq_r > 0.0).astype(float)
                ...
                robust_rows.append(
                    {
                        ...
                        "ConvictionMultiplier": conviction_mult,
                        "LeaderMultiplier": leader_part_mult,
                        "CAGR": summary["CAGR"],
...
def _event_window_return(r: pd.Series, dt: pd.Timestamp, horizon: int) -> float:
    s = pd.Series(r, dtype=float).sort_index()
...
def _event_window_drawdown(eq: pd.Series, dt: pd.Timestamp, horizon: int) -> float:
    s = pd.Series(eq, dtype=float).sort_index().ffill()
    if dt not in s.index:
        return np.nan
    loc = s.index.get_loc(dt)
    if isinstance(loc, slice):
...
def _continuation_variant_row(
    result: Dict[str, Any],
    cfg: Mahoraga14Config,
    variant_key: str,
) -> Dict[str, Any]:
    weekly = result["weekly_full"].copy()
    override_weekly = result["variant_runs"][variant_key]["override_weekly"].copy().reindex(weekly.index).fillna(0.0)
    override_daily = result["variant_runs"][variant_key]["override_daily"].loc[result["test_start"] : result["test_end"]].copy()
    bt = result["variant_bts"][variant_key]
    qqq_r = result["base_bt"]["bench"]["QQQ_r"].loc[result["test_start"] : result["test_end"]]
    spy_r = result["base_bt"]["bench"]["SPY_r"].loc[result["test_start"] : result["test_end"]]

    active_dates = weekly.index[override_weekly.get("is_continuation_activation", pd.Series(0.0, index=weekly.index)) > 0.0]
    inactive_dates = weekly.index.difference(active_dates)

    def _collect(dates: pd.Index) -> pd.DataFrame:
        rows = []
        for dt in dates:
            rows.append(
                {
                    "ret1w": _event_window_return(bt["returns_net"], dt, 5),
                    "ret4w": _event_window_return(bt["returns_net"], dt, 20),
                    "qqq4w": _event_window_return(qqq_r, dt, 20),
                    "spy4w": _event_window_return(spy_r, dt, 20),
                    "dd4w": _event_window_drawdown(bt["equity"], dt, 20),
                }
            )
        return pd.DataFrame(rows)

    active = _collect(active_dates)
    inactive = _collect(inactive_dates)
    pressure_series = weekly.get("continuation_pressure_score", pd.Series(0.0, index=weekly.index))
    trigger_series = weekly.get("continuation_trigger_score", pd.Series(0.0, index=weekly.index))
    break_series = weekly.get("continuation_break_risk_p", pd.Series(0.0, index=weekly.index))
    benchmark_series = weekly.get("continuation_benchmark_score", pd.Series(0.0, index=weekly.index))
    lift_rate = float(override_daily.get("is_continuation_lift", pd.Series(0.0, index=override_daily.index)).mean()) if len(override_daily) else 0.0

    active_rate = float(len(active_dates) / max(1, len(weekly.index)))
    edge_vs_noact = float(active["ret4w"].mean() - inactive["ret4w"].mean()) if len(active) and len(inactive) else 0.0
    useful = int(bool(len(active)) and float((active["ret4w"] > 0.0).mean()) >= float((inactive["ret4w"] > 0.0).mean()) and edge_vs_noact > 0.0)
    label = _label_map(cfg)[variant_key]
    return {
        "Segment": f"FOLD_{int(result['fold'])}",
        "Fold": int(result["fold"]),
        "Variant": label,
        "Activations": int(len(active_dates)),
        "ActivationRate": round(active_rate, 6),
        "LiftRate": round(lift_rate, 6),
        "HitRate1W": round(float((active["ret1w"] > 0.0).mean()) if len(active) else 0.0, 6),
        "HitRate4W": round(float((active["ret4w"] > 0.0).mean()) if len(active) else 0.0, 6),
        "NoActHitRate4W": round(float((inactive["ret4w"] > 0.0).mean()) if len(inactive) else 0.0, 6),
        "MeanRet1W": round(float(active["ret1w"].mean()) if len(active) else 0.0, 8),
        "MeanRet4W": round(float(active["ret4w"].mean()) if len(active) else 0.0, 8),
        "MeanExcessVsQQQ4W": round(float((active["ret4w"] - active["qqq4w"]).mean()) if len(active) else 0.0, 8),
        "MeanExcessVsSPY4W": round(float((active["ret4w"] - active["spy4w"]).mean()) if len(active) else 0.0, 8),
        "MeanPostDD4W": round(float(active["dd4w"].mean()) if len(active) else 0.0, 8),
        "EdgeVsNoActivation4W": round(edge_vs_noact, 8),
        "MeanTriggerScore": round(float(trigger_series.reindex(active_dates).mean()) if len(active_dates) else 0.0, 6),
        "MeanPressureScore": round(float(pressure_series.reindex(active_dates).mean()) if len(active_dates) else 0.0, 6),
        "MeanBreakRisk": round(float(break_series.reindex(active_dates).mean()) if len(active_dates) else 0.0, 6),
        "MeanBenchmarkScore": round(float(benchmark_series.reindex(active_dates).mean()) if len(active_dates) else 0.0, 6),
        "PressureEntry": round(float(result["continuation_fit"].get("pressure_enter", np.nan)), 6),
        "BreakRiskCap": round(float(result["continuation_fit"].get("break_risk_cap", np.nan)), 6),
        "ContinuationUseful": useful,
    }


def _build_continuation_diagnostic(wf: Dict[str, Any], cfg: Mahoraga14Config) -> pd.DataFrame:
    rows = []
    for result in wf["results"]:
        for variant_key in (cfg.control_variant_key, cfg.primary_variant_key):
            rows.append(_continuation_variant_row(result, cfg, variant_key))
    df = pd.DataFrame(rows)
    if len(df) == 0:
        return df
    ...
    for variant, sub in df.groupby("Variant"):
        act_weights = sub["Activations"].clip(lower=1)
        fold_weights = sub["ActivationRate"].clip(lower=1e-6)
        stitched_rows.append(
            {
                ...
                "ActivationRate": round(float(np.average(sub["ActivationRate"], weights=fold_weights)), 6),
                "LiftRate": round(float(np.average(sub["LiftRate"], weights=fold_weights)), 6),
                "HitRate1W": round(float(np.average(sub["HitRate1W"], weights=act_weights)), 6),
                "HitRate4W": round(float(np.average(sub["HitRate4W"], weights=act_weights)), 6),
                "NoActHitRate4W": round(float(np.average(sub["NoActHitRate4W"], weights=sub["ActivationRate"].rsub(1.0).clip(lower=1e-6))), 6),
                "MeanRet1W": round(float(np.average(sub["MeanRet1W"], weights=act_weights)), 8),
                "MeanRet4W": round(float(np.average(sub["MeanRet4W"], weights=act_weights)), 8),
                "MeanExcessVsQQQ4W": round(float(np.average(sub["MeanExcessVsQQQ4W"], weights=act_weights)), 8),
                "MeanExcessVsSPY4W": round(float(np.average(sub["MeanExcessVsSPY4W"], weights=act_weights)), 8),
                "MeanPostDD4W": round(float(np.average(sub["MeanPostDD4W"], weights=act_weights)), 8),
                "EdgeVsNoActivation4W": round(float(np.average(sub["EdgeVsNoActivation4W"], weights=act_weights)), 8),
                "MeanTriggerScore": round(float(np.average(sub["MeanTriggerScore"], weights=act_weights)), 6),
                "MeanPressureScore": round(float(np.average(sub["MeanPressureScore"], weights=act_weights)), 6),
                "MeanBreakRisk": round(float(np.average(sub["MeanBreakRisk"], weights=act_weights)), 6),
                "MeanBenchmarkScore": round(float(np.average(sub["MeanBenchmarkScore"], weights=act_weights)), 6),
                "PressureEntry": round(float(np.average(sub["PressureEntry"].fillna(0.0), weights=fold_weights)), 6),
                "BreakRiskCap": round(float(np.average(sub["BreakRiskCap"].fillna(0.0), weights=fold_weights)), 6),
                "ContinuationUseful": int(sub["ContinuationUseful"].mean() >= 0.5),
            }
        )
    return pd.concat([df, pd.DataFrame(stitched_rows)], ignore_index=True).round(8)
...
def _fast_fail_status(
    ...
    if upside_delta < 0.02 and exposure_delta < 0.015:
    ...
    if primary["UpsideCaptureQQQ"] <= control["UpsideCaptureQQQ"]:
        reasons.append("upside_capture_qqq_not_improved")
    ...
    if len(manual):
        ...
        priority = manual[manual["Window"].isin(["2017_2018", "2020_2021"])].copy()
        if len(priority) and bool((priority["DeltaReturn_vs_Control"] <= 0.0).any()):
            reasons.append("priority_bull_windows_not_improved")
    ...
    if primary["AlphaNW_QQQ"] <= control["AlphaNW_QQQ"] and primary["AlphaNW_SPY"] <= control["AlphaNW_SPY"] and primary["CAGR"] <= control["CAGR"]:
        reasons.append("no_clear_economic_improvement_vs_benchmarks")
...
def _candidate_audit(
    ...
    primary_mask = rows["Variant"] == "MAHORAGA14_3_LONG_PARTICIPATION"
    ...
    if len(scorecard_df):
        ...
        priority = scorecard_df[scorecard_df["Window"].isin(["2017_2018", "2020_2021"])].copy()
        if len(priority):
            rows.loc[primary_mask, "PriorityBullDelta_vs_Control"] = float(priority["DeltaReturn_vs_Control"].mean())
    if len(stress_df):
        non_base = stress_df[stress_df["Scenario"] != "BASELINE"].copy()
...
def _final_report_md(
    ...
    stress_df: pd.DataFrame,
    robustness_df: pd.DataFrame,
    continuation_diag_df: pd.DataFrame,
    status: str,
    reasons: List[str],
) -> str:
    primary = comparison_df[comparison_df["Variant"] == "MAHORAGA14_3_LONG_PARTICIPATION"].iloc[0]
    ...
    lines += [
        ...
        _frame_block(continuation_diag_df) if len(continuation_diag_df) else "No continuation diagnostic available.",
        "",
...
def build_fast_report_text(wf: Dict[str, Any], cfg: Mahoraga14Config) -> str:
    ...
    continuation_diag_df = _build_continuation_diagnostic(wf, cfg)
    status, reasons = _fast_fail_status(comparison_df, scorecard_df, stress_df, cfg)
    return _final_report_md(comparison_df, scorecard_df, decomposition_df, stress_df, robustness_df, continuation_diag_df, status, reasons)
...
def save_fast_outputs(wf: Dict[str, Any], cfg: Mahoraga14Config, costs: m6.CostsConfig | None = None) -> Dict[str, pd.DataFrame]:
    ...
    final_md = _final_report_md(comparison_df, scorecard_df, decomposition_df, stress_df, robustness_df, continuation_diag_df, status, reasons)

    out_dir = Path(cfg.outputs_dir)
    comparison_df.to_csv(out_dir / "stitched_comparison_fast_14_3.csv", index=False)
    scorecard_df.to_csv(out_dir / "bull_window_scorecard_fast_14_3.csv", index=False)
    active_df.to_csv(out_dir / "active_return_vs_qqq_fast_14_3.csv", index=False)
    curve_df.to_csv(out_dir / "active_return_vs_qqq_curve_14_3.csv", index=False)
    decomposition_df.to_csv(out_dir / "upside_participation_decomposition_fast_14_3.csv", index=False)
    allocator_cash_drag_df.to_csv(out_dir / "allocator_cash_drag_fast_14_3.csv", index=False)
    leader_miss_df.to_csv(out_dir / "leader_miss_analysis_fast_14_3.csv", index=False)
    continuation_diag_df.to_csv(out_dir / "continuation_diagnostic_fast_14_3.csv", index=False)
    candidate_audit_df.to_csv(out_dir / "candidate_audit_fast_14_3.csv", index=False)
    stress_df.to_csv(out_dir / "stress_suite_fast_14_3.csv", index=False)
    robustness_df.to_csv(out_dir / "robustness_suite_fast_14_3.csv", index=False)
    _curve_plot(curve_df, out_dir / "active_return_vs_qqq_curve_14_3.png")
    with open(out_dir / "final_report_fast_14_3.md", "w", encoding="utf-8") as f:
        f.write(final_md)

    return {
        "stitched_comparison_fast_14_3": comparison_df,
        "bull_window_scorecard_fast_14_3": scorecard_df,
        "active_return_vs_qqq_fast_14_3": active_df,
        "active_return_vs_qqq_curve_14_3": curve_df,
        "upside_participation_decomposition_fast_14_3": decomposition_df,
        "allocator_cash_drag_fast_14_3": allocator_cash_drag_df,
        "leader_miss_analysis_fast_14_3": leader_miss_df,
        "continuation_diagnostic_fast_14_3": continuation_diag_df,
        "candidate_audit_fast_14_3": candidate_audit_df,
        "stress_suite_fast_14_3": stress_df,
        "robustness_suite_fast_14_3": robustness_df,
    }
