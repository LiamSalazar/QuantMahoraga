...
def _ema_slope(price: pd.Series, span: int, lag: int) -> pd.Series:
    ema = price.ewm(span=span, adjust=False).mean()
    return (ema / ema.shift(lag) - 1.0).replace([np.inf, -np.inf], np.nan).fillna(0.0)


def _trend_state(price: pd.Series, fast: int, slow: int) -> pd.Series:
    fast_ema = price.ewm(span=fast, adjust=False).mean()
    slow_ema = price.ewm(span=slow, adjust=False).mean()
    state = 0.5 * (price > fast_ema).astype(float) + 0.5 * (fast_ema > slow_ema).astype(float)
    return state.reindex(price.index).fillna(0.0)
...
def _drawdown_duration(eq: pd.Series) -> pd.Series:
    ...
    duration = (~at_high).groupby(groups).cumsum()
...
def build_market_path_context(
    ...
    out["qqq_vol_63"] = (qqq_r.rolling(63).std(ddof=1) * np.sqrt(cfg.trading_days)).fillna(0.0)
    out["qqq_slope_21"] = _ema_slope(qqq, span=21, lag=5)
    out["qqq_slope_63"] = _ema_slope(qqq, span=63, lag=10)
    out["qqq_trend_state"] = _trend_state(qqq, fast=21, slow=63)
...
def build_candidate_daily_context(
    ...
    base_dd_duration = _drawdown_duration(base_eq)
    ...
    out["base_dd_duration"] = base_dd_duration
    out["base_dd_velocity_10d"] = base_dd.diff(10).fillna(0.0)
    out["base_rebound_10d"] = _rebound_from_trough(base_eq, 10)
    out["base_rebound_20d"] = _rebound_from_trough(base_eq, 20)
    out["base_rebound_asym_20d"] = (_rebound_from_trough(base_eq, 20) + base_dd.diff(10).fillna(0.0)).fillna(0.0)
    out["base_eff_10d"] = _rolling_efficiency(base_r, 10)
    out["base_eff_20d"] = _rolling_efficiency(base_r, 20)
    out["base_realized_vol_21"] = (base_r.rolling(21).std(ddof=1) * np.sqrt(252)).fillna(0.0)
    out["base_slope_10d"] = _ema_slope(base_eq, span=10, lag=5)
    out["base_slope_20d"] = _ema_slope(base_eq, span=20, lag=10)
    out["base_trend_state"] = _trend_state(base_eq, fast=10, slow=20)
    ...
    out["stop_release_10d"] = (-stop_active.rolling(10).mean().diff(5)).fillna(0.0)
...
def build_weekly_path_dataset(daily_ctx: pd.DataFrame, cfg: Mahoraga14Config) -> pd.DataFrame:
    ...
    weekly["base_dd_duration_2w"] = weekly["base_dd_duration"].rolling(2).mean().fillna(0.0) / 5.0
    weekly["base_dd_duration_4w"] = weekly["base_dd_duration"].rolling(4).mean().fillna(0.0) / 5.0
    weekly["base_dd_velocity_2w"] = weekly["base_dd_velocity_10d"].rolling(2).mean().fillna(0.0)
    weekly["base_dd_velocity_4w"] = weekly["base_dd_velocity_10d"].rolling(4).mean().fillna(0.0)
    ...
    weekly["base_rebound_asym_4w"] = weekly["base_rebound_asym_20d"].rolling(4).mean().fillna(0.0)
    ...
    weekly["base_realized_vol_4w"] = weekly["base_realized_vol_21"].rolling(4).mean().fillna(0.0)
    weekly["base_slope_2w"] = weekly["base_slope_10d"].rolling(2).mean().fillna(0.0)
    weekly["base_slope_4w"] = weekly["base_slope_20d"].rolling(4).mean().fillna(0.0)
    weekly["base_trend_state_4w"] = weekly["base_trend_state"].rolling(4).mean().fillna(0.0)
    ...
    weekly["stop_release_2w"] = weekly["stop_release_10d"].rolling(2).mean().fillna(0.0)
    ...
    weekly["scale_recovery_2w"] = weekly["scale_recovery_10d"].rolling(2).mean().fillna(0.0)
    ...
    weekly["qqq_slope_2w"] = weekly["qqq_slope_21"].rolling(2).mean().fillna(0.0)
    weekly["qqq_slope_4w"] = weekly["qqq_slope_63"].rolling(4).mean().fillna(0.0)
    weekly["qqq_trend_state_4w"] = weekly["qqq_trend_state"].rolling(4).mean().fillna(0.0)
    weekly["qqq_vol_4w"] = weekly["qqq_vol_21"].rolling(4).mean().fillna(0.0)
    weekly["qqq_vol_12w"] = weekly["qqq_vol_63"].rolling(12).mean().fillna(0.0)
