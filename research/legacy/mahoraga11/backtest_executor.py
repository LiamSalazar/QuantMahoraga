...
from mahoraga11_utils import bhy_qvalues, iter_grid, paired_ttest_pvalue, time_split_index
...
def _prepare_fold_invariants(
    ...
    alpha_fit = fit_base_alpha_model(close, qqq, cfg, train_start, train_end)
...
def _slice_pre(pre: Dict[str, Any], end: pd.Timestamp) -> Dict[str, Any]:
    idx = pre["idx"][pre["idx"] <= end]
    sliced = dict(pre)
    for key in ["close", "high", "low", "rets", "qqq", "spy", "crisis_scale", "crisis_state", "turb_scale", "corr_rho", "corr_scale_legacy", "corr_state"]:
        sliced[key] = pre[key].loc[idx]
    sliced["idx"] = idx
    sliced["reb_dates"] = {dt for dt in pre["reb_dates"] if dt <= end}
    sliced["alpha_fit"] = pre["alpha_fit"]
    return sliced


def _slice_components(components: Dict[str, Any], idx: pd.DatetimeIndex) -> Dict[str, Any]:
    out: Dict[str, Any] = {}
    for key, value in components.items():
        out[key] = value.loc[idx] if hasattr(value, "loc") else value
    return out


def _build_legacy_baseline_bt(pre: Dict[str, Any], cfg: Mahoraga11Config, costs: m6.CostsConfig) -> Dict[str, Any]:
...
def _summarize_window(bt: Dict[str, Any], start: pd.Timestamp, end: pd.Timestamp, cfg: Mahoraga11Config, label: str) -> Dict[str, float]:
...
def _candidate_metrics(
    ...
    if fold_n in cfg.ceiling_folds:
        utility = (
            1.45 * base_vs_legacy_sharpe
            + 0.40 * base_vs_legacy_cagr
            + 0.25 * base_vs_legacy_dd
            + 0.45 * model_vs_base_sharpe
            + 0.10 * model_vs_base_cagr
            + 0.12 * model_vs_base_dd
            - 2.30 * max(0.0, -base_vs_legacy_sharpe - 0.03)
            - 1.20 * max(0.0, -base_vs_legacy_cagr - 0.01)
            - 1.60 * max(0.0, -model_vs_base_sharpe - 0.02)
            - 0.70 * max(0.0, override_rate - 0.20)
            - 0.50 * mean_defense_blend
        )
    else:
        utility = (
            0.75 * base_vs_legacy_sharpe
            + 0.25 * base_vs_legacy_cagr
            + 0.20 * base_vs_legacy_dd
            + 1.35 * model_vs_base_sharpe
            + 0.40 * model_vs_base_cagr
            + 0.35 * model_vs_base_dd
            - 1.20 * max(0.0, -base_vs_legacy_sharpe - 0.10)
            - 0.90 * max(0.0, -model_vs_base_sharpe - 0.06)
            - 0.35 * max(0.0, override_rate - 0.45)
            - 0.20 * max(0.0, structural_rate - 0.25)
        )
...
def _run_single_fold(
    ...
    pre_cal = _slice_pre(pre, train_end)
    ...
    components_cal = _slice_components(pre["components"], pre_cal["idx"])
    ...
    for eng in engine_candidates:
        ...
        for policy_params in policy_candidates:
            ...
            override_daily = weekly_to_daily_override(override_weekly, pre_cal["idx"])
            ...
            if fold_n in cfg_fold.ceiling_folds:
                metrics["utility"] -= 0.25 * float(eng["base_mix"]) + 0.10 * max(0.0, float(eng["raw_rel_boost"]) - 1.0)
            else:
                metrics["utility"] -= 0.10 * float(eng["base_mix"])
            leaderboard.append(
                {
                    **eng,
                    **policy_params,
    ...
    override_daily = weekly_to_daily_override(override_weekly_full, pre["idx"])
    ...
    s_model = _summarize_window(model_bt, test_start, test_end, cfg_fold, f"M11_{fold_n}")
    ...
    fold_row = {
        ...
        "M11_CAGR%": round(s_model["CAGR"] * 100, 2),
        "M11_Sharpe": round(s_model["Sharpe"], 4),
        "M11_MaxDD%": round(s_model["MaxDD"] * 100, 2),
        "StructuralModel": structural_fit["name"],
        "TransitionModel": transition_fit["name"],
        "RecoveryModel": recovery_fit["name"],
...
def run_walk_forward_mahoraga11(
    ohlcv: Dict[str, pd.DataFrame],
    cfg: Mahoraga11Config,
    costs: m6.CostsConfig,
    universe_schedule: Optional[pd.DataFrame],
    ...
    stitched_model = _stitch(results, "m11_bt")
    return {
        "results": results,
        "fold_df": fold_df,
