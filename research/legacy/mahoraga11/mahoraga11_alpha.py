...
from mahoraga11_utils import cross_sectional_z
...
def fit_alpha_model(close: pd.DataFrame, qqq: pd.Series, cfg: Mahoraga11Config, train_start: pd.Timestamp, train_end: pd.Timestamp) -> Dict[str, Any]:
...
def build_alpha_components(close: pd.DataFrame, qqq: pd.Series, cfg: Mahoraga11Config) -> Dict[str, pd.DataFrame]:
    idx = close.index
...
def build_engine_score(components: Dict[str, pd.DataFrame], alpha_fit: Dict[str, Any], mix: float, beta_penalty: float, raw_rel_boost: float, cfg: Mahoraga11Config) -> pd.DataFrame:
    rw = np.array(alpha_fit["raw_weights"], dtype=float)
    rw = np.array([rw[0], rw[1], rw[2] * raw_rel_boost], dtype=float)
    ...
    beta_pen = (components["beta_df"] - 1.0).clip(lower=0.0) * beta_penalty
...
def precompute_engine_path(pre: Dict[str, Any], components: Dict[str, pd.DataFrame], alpha_fit: Dict[str, Any], mix: float, beta_penalty: float, raw_rel_boost: float, cfg: Mahoraga11Config) -> Dict[str, Any]:
    idx = pre["idx"]
    close = pre["close"]
    high = pre["high"]
    low = pre["low"]
