...
def annotate_structural_labels(weekly_df: pd.DataFrame, train_end: pd.Timestamp) -> pd.DataFrame:
    ...
    dd_duration_hi = float(out.loc[train_idx, "base_dd_duration_4w"].dropna().quantile(0.65)) if len(train_idx) else 3.0
    ...
    out["structural_y"] = (
        ...
        | (dd4 <= dd4_low)
        | (rel4 <= rel4_low)
        | ((out["base_dd_duration_4w"] >= dd_duration_hi) & (fwd4 < 0.0) & (out["base_rebound_4w"] <= 0.01))
    ).astype(int)
    return out
