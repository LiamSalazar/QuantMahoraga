...
def build_final_report_text(wf: Dict[str, Any], cfg: Mahoraga12Config) -> str:
    if cfg.run_mode.upper() == "FULL":
        return build_full_report_text(wf, cfg)
    return build_fast_report_text(wf, cfg)


def save_outputs(wf: Dict[str, Any], cfg: Mahoraga12Config, ff=None) -> Dict[str, pd.DataFrame]:
    if cfg.run_mode.upper() == "FULL":
