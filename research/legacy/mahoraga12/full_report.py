...
def save_full_outputs(wf: Dict[str, Any], cfg: Mahoraga12Config, ff=None) -> Dict[str, pd.DataFrame]:
    ...
    if len(override_weekly):
        override_diag = (
            ...
            .agg(
                ...
                MeanContinuationScore=("continuation_score", "mean"),
            )
        )
    ...
    stitched_trace = wf["stitched_test_trace"].copy()
    stitched_trace.to_csv(f"{cfg.outputs_dir}/stitched_test_trace.csv", index=False)
    ...
    if ff is not None:
        ff_rows = [
            m6.factor_attribution(base_r, ff, cfg, label=cfg.official_baseline_label),
            m6.factor_attribution(model_r, ff, cfg, label=cfg.model_label),
        ]
