...
Performance fix (PERF-7C2) — applied in this file
---------------------------------------------------
`_policy_from_dual_probs` was a Python for-loop over weekly dates (~260 iter/call),
called 320 times per fold (Phase A: 64, Phase B: 256). ~83,200 Python iterations.
Each call also recomputed 10 quantile thresholds, of which 8 are fixed cfg constants
(never change within a fold's grid sweep) and only 2 vary with grid params
(stress_trigger_q, recovery_trigger_q — 4 unique pairs per outer loop).

Fixed in three steps:
1. `_precompute_fixed_thresholds`: compute the 8 cfg-constant thresholds once per
   outer pair. Cache (stress_thr, rec_thr) for the 4 (stq, rtq) pairs separately.
2. `_policy_from_dual_probs_vec`: fully vectorized replacement using numpy arrays —
   same 5-condition recovery gate, same 4-branch allow_recover, same 4-component
   rec_scale (including momentum_relief), same recover_cond_count output column.
   Zero Python-level iteration over dates.
3. `_policy_from_dual_probs` retained as a thin wrapper for _run_single_fold (test
   phase, called once — precomputing externally would add complexity with no benefit).

Bug fixes in this file:
- Corrected 3 print labels from "7C.2" to "7C.4" in _run_single_fold.
- Removed dead `for ... pass` loop (leftover from 7C.2 transition).

Result: ~83,200 Python iterations → 320 numpy array ops (~100x faster on policy section).
        2,560 nanquantile calls → 40 (~64x fewer).
"""
...
DISCLAIMER = r"""
═══════════════════════════════════════════════════════════════════════════════
  MAHORAGA 7C.4 — HAWKES + DUAL-ML LAYER OVER MAHORAGA 6.1
───────────────────────────────────────────────────────────────────────────────
...
@dataclass
class Mahoraga7C3Config(h7.Mahoraga7Config):
    variant: str = "7C.4"
    outputs_dir: str = "mahoraga7c4_outputs"
    plots_dir: str = "mahoraga7c4_plots"
    label: str = "MAHORAGA_7C4"
...
def _load_baseline_folds(cfg: Mahoraga7C3Config) -> pd.DataFrame:
...
def _future_window_utility(
    ...
    out_vals = np.full(len(weekly_idx), np.nan)
    if len(idx) == 0:
        return pd.Series(out_vals, index=weekly_idx, dtype=float)
    step = max(1, horizon_weeks * 5)
    # Precompute cumulative log-returns and raw equity arrays for fast slicing
    r_vals = returns_daily.values.astype(float)
    eq_vals = equity_daily.values.astype(float)
    cumlog = np.empty(len(r_vals) + 1)
    cumlog[0] = 0.0
    np.cumsum(np.log1p(r_vals), out=cumlog[1:])
    starts = np.searchsorted(idx, weekly_idx, side="left")
    for i in range(len(weekly_idx)):
        start_i = int(starts[i])
        if start_i >= len(idx):
            continue
        end_i = min(len(idx) - 1, start_i + step)
        if end_i <= start_i:
            continue
        ret = float(np.expm1(cumlog[end_i + 1] - cumlog[start_i + 1]))
        eq_seg = eq_vals[start_i:end_i + 1]
        if len(eq_seg) <= 1:
            dd = 0.0
        else:
            running_max = np.maximum.accumulate(eq_seg)
            dd = float(np.min(eq_seg / running_max) - 1.0)
        out_vals[i] = ret - dd_penalty * abs(dd)
    return pd.Series(out_vals, index=weekly_idx, dtype=float)
...
def _build_ml_feature_table(hawkes_df: pd.DataFrame, base_bt: Dict[str, Any], cfg: Mahoraga7C3Config) -> pd.DataFrame:
...
def _fit_binary_model(X: pd.DataFrame, y: pd.Series, cfg: Mahoraga7C3Config):
    if not _SKLEARN:
        return None
    vc = y.value_counts()
...
def _future_window_return(returns_daily: pd.Series, weekly_idx: pd.DatetimeIndex, horizon_weeks: int) -> pd.Series:
    ...
    if len(idx) == 0:
        return pd.Series(out_vals, index=weekly_idx, dtype=float)
    step = max(1, horizon_weeks * 5)
    # Precompute cumulative log returns for O(1) window product
    log_r = np.log1p(returns_daily.values.astype(float))
    cumlog = np.empty(len(log_r) + 1)
    cumlog[0] = 0.0
    np.cumsum(log_r, out=cumlog[1:])
    starts = np.searchsorted(idx, weekly_idx, side="left")
    ends = np.minimum(len(idx) - 1, starts + step)
    valid = (starts < len(idx)) & (ends > starts)
    vs, ve = starts[valid], ends[valid]
    # r_seg runs from pos+1 to end_i inclusive → cumlog[ve+1] - cumlog[vs+1]
    out_vals[valid] = np.expm1(cumlog[ve + 1] - cumlog[vs + 1])
    return pd.Series(out_vals, index=weekly_idx, dtype=float)
...
def _precompute_fixed_thresholds(
    ...
    """
    7C.4: extend the reusable threshold bundle with panic-state diagnostics.
    These thresholds are derived only from the training slice and reused across
    all policy evaluations inside the fold.
    """
...
def _policy_from_dual_probs_vec(
    ...
    7C.4 policy engine.
    Adds:
      - explicit panic detection from stress/correlation/VIX/drawdown context,
      - cooldown after severe panic states,
      - stepwise recovery override (LIGHT/FULL) while keeping the external action
        vocabulary unchanged,
      - stronger tail defence without replacing the frozen 6.1 baseline.
    """
    idx = weekly_df.index
    ...
    cond_count = (
        cond_rec.astype(int)
        + cond_spread.astype(int)
        + cond_scale.astype(int)
        + cond_breadth.astype(int)
        + cond_rel.astype(int)
    )

    recover_opp = (pr >= recovery_prob_trigger) & ~chop & (cond_count >= cfg.recovery_min_conditions)
    can_frag = (pf >= prob_trigger) & ~(chop & ~crisis_on)
    ...
    strong_recover = allow_recover & (
        ...
        & (
            (spread >= spread_pos_thr * cfg.recovery_mode_full_spread_mult)
            | (rec > stress)
        )
    ...
    rel_confirm = (rel_top >= rel_thr_f) & ((rm_mom > 0) | (rm_tr > 0)) & ~crisis_on & ~panic_mode
    ...
    # Cooldown: after panic, keep the system partially defensive for a small
    # number of decision periods unless recovery evidence becomes strong.
    cooldown_active = np.zeros(len(idx), dtype=bool)
    cooldown = 0
    for i in range(len(idx)):
        trigger = bool(panic_mode[i]) and action[i] != "RECOVERY_OVERRIDE"
    ...
    reduce = np.where(~crisis_on & ~rec_prob_hi, 0.95, 1.0)
    rec_scale_raw = rec_scale_raw * reduce
    rec_scale_full = np.clip(rec_scale_raw, recovery_floor, cfg.recovery_floor_cap)
    rec_scale_light = np.clip(
        recovery_floor + cfg.recovery_light_blend * np.maximum(0.0, rec_scale_full - recovery_floor),
        recovery_floor,
        cfg.recovery_floor_cap,
    )
    rec_scale_arr = np.where(
        action == "RECOVERY_OVERRIDE",
        np.where(strong_recover, rec_scale_full, rec_scale_light),
        0.0,
    )

    rel_tilt_arr = np.where(action == "REL_TILT", rel_tilt, np.nan)
    recovery_mode = np.where(action != "RECOVERY_OVERRIDE", "", np.where(strong_recover, "FULL", "LIGHT"))

    return pd.DataFrame({
        "fragility_prob": pf,
        "recovery_prob": pr,
        "action": action,
        "ext_scale": ext_scale_arr.astype(float),
        "recovery_override_scale": rec_scale_arr.astype(float),
        "rel_tilt": rel_tilt_arr.astype(float),
        "stress_thr": stress_thr,
        "recovery_thr": rec_thr,
        "prob_trigger": prob_trigger,
        "recovery_prob_trigger": recovery_prob_trigger,
        "recover_opp": recover_opp.astype(int),
        "recover_fire": (action == "RECOVERY_OVERRIDE").astype(int),
        "frag_fire": (action != "BASELINE").astype(int) * can_frag.astype(int),
        "chop": chop.astype(int),
        "recover_cond_count": cond_count,
        "panic_mode": panic_mode.astype(int),
        "panic_cond_count": panic_count.astype(int),
...
def _compute_weights_and_vol(
    ...
    """
    PERF-7C2 Phase A: compute the weight trajectory and vol scale for one
...
def _score_from_weights(
    ...
    """
    7C.4 scoring path.
    Adds explicit tail / panic / missed-rebound terms while keeping the fast
    weight reuse introduced in 7C.3.
    """
    idx = ctx["idx"]
    rets = ctx["rets"]
...
def _run_single_fold(
    ...
    print(f"\n  ── 7C.4 FOLD {fold_n}/{total_folds} ──")
    cfg_fold = _get_fold_cfg(ohlcv, cfg_base, costs, universe_schedule, baseline_row)
    print(f"  [fold {fold_n}] Calibrating 7C.4 on train via inner validation …")
    ...
    ov_bt = h7._custom_backtest_with_overlay(ohlcv, cfg_fold, costs, universe_schedule, policy_daily, label=f"H7C4_{fold_n}", ctx=ctx_test, score_rel_cache={})
    ...
    so = m6.summarize(ro, qo, eo, to, cfg_fold, f"H7C4_{fold_n}")
    ...
    print(f"  [fold {fold_n}] BASE Sharpe={sb['Sharpe']:.3f} | 7C.4 Sharpe={so['Sharpe']:.3f} | Δ={so['Sharpe'] - sb['Sharpe']:+.3f}")
    ...
    return {
        ...
        "fold_row": {
            ...
            "H7C4_CAGR%": round(so["CAGR"] * 100, 2),
            "H7C4_Sharpe": round(so["Sharpe"], 4),
            "H7C4_MaxDD%": round(so["MaxDD"] * 100, 2),
            "H7C4_CVaR5%": round(so["CVaR_5"] * 100, 2),
            ...
            "ChopRate": round(float(policy_weekly.get("chop", pd.Series(0.0, index=policy_weekly.index)).mean()), 4),
        },
...
def run_walk_forward_h7c4(
    ...
    # When not parallelising across folds, free up all cores for the RF fits
    if not use_parallel and cfg.rf_n_jobs == 1:
        cfg.rf_n_jobs = -1
...
def _selection_audit(wf: Dict[str, Any], cfg: Mahoraga7C3Config) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        ...
        {
            "Method": "H7C4",
            "MeanSharpe": fr["H7C4_Sharpe"].mean(),
            "MeanCAGR%": fr["H7C4_CAGR%"].mean(),
            "MeanMaxDD%": fr["H7C4_MaxDD%"].mean(),
...
def _regime_comparison(base_r: pd.Series, ov_r: pd.Series, cfg: Mahoraga7C3Config, ohlcv: Dict[str, pd.DataFrame]) -> pd.DataFrame:
...
def _final_report_text(
    ...
    wf: Dict[str, Any],
    ohlcv: Dict[str, pd.DataFrame],
    base_sum: Optional[Dict[str, Any]] = None,
    ov_sum: Optional[Dict[str, Any]] = None,
    sel: Optional[pd.DataFrame] = None,
    reg: Optional[pd.DataFrame] = None,
    alpha_base: Optional[Any] = None,
    alpha_ov: Optional[Any] = None,
    alpha_ov_cond: Optional[Any] = None,
) -> str:
    if base_sum is None:
        base_sum = m6.summarize(wf["base_oos_r"], wf["base_oos_eq"], wf["base_oos_exp"], wf["base_oos_to"], cfg, "BASE_OOS")
    if ov_sum is None:
        ov_sum = m6.summarize(wf["ov_oos_r"], wf["ov_oos_eq"], wf["ov_oos_exp"], wf["ov_oos_to"], cfg, "H7C4_OOS")
    if sel is None:
    ...
    if alpha_ov is None:
        alpha_ov = m6.alpha_test_nw(wf["ov_oos_r"], wf["qqq_oos_r"], cfg, label="H7C4_OOS")
    if alpha_ov_cond is None:
        alpha_ov_cond = m6.alpha_test_nw(wf["ov_oos_r"], wf["qqq_oos_r"], cfg, label="H7C4_OOS|exp>0", conditional=True, exposure=wf["ov_oos_exp"])
    fr = wf["fold_results"]
    lines = []
    lines.append("MAHORAGA 7C.4 — FINAL REPORT")
    lines.append("=" * 78)
    ...
    lines.append(f"  H7C4    CAGR={ov_sum['CAGR']*100:.2f}%  Sharpe={ov_sum['Sharpe']:.3f}  MaxDD={ov_sum['MaxDD']*100:.2f}%  CVaR5={ov_sum['CVaR_5']*100:.2f}%")
...
def save_outputs_h7c4(cfg: Mahoraga7C3Config, wf: Dict[str, Any], ohlcv: Dict[str, pd.DataFrame]):
    d = cfg.outputs_dir
    _ensure_dir(d)
    base_sum = m6.summarize(wf["base_oos_r"], wf["base_oos_eq"], wf["base_oos_exp"], wf["base_oos_to"], cfg, "BASE_OOS")
    ov_sum = m6.summarize(wf["ov_oos_r"], wf["ov_oos_eq"], wf["ov_oos_exp"], wf["ov_oos_to"], cfg, "H7C4_OOS")
    ...
    pd.DataFrame([alpha_base, alpha_ov, alpha_ov_cond]).to_csv(os.path.join(d, "alpha_comparison.csv"), index=False)
    wf["fold_results"].to_csv(os.path.join(d, "walk_forward_folds_7c4.csv"), index=False)
    ...
    sel.to_csv(os.path.join(d, "selection_audit.csv"), index=False)
    reg.to_csv(os.path.join(d, "regime_comparison.csv"), index=False)
    with open(os.path.join(d, "final_report.txt"), "w", encoding="utf-8") as f:
        f.write(_final_report_text(cfg, wf, ohlcv, base_sum, ov_sum, sel, reg, alpha_base, alpha_ov, alpha_ov_cond))
    print(f"\n  [outputs → ./{d}/]")
    print("    comparison_oos.csv, comparison_full.csv, alpha_comparison.csv, walk_forward_folds_7c4.csv")
...
def run_mahoraga7c4(make_plots_flag: bool = False, run_mode: str = "FULL") -> Dict[str, Any]:
    if not _SKLEARN:
        raise RuntimeError("scikit-learn is required for Mahoraga 7C")
    print("=" * 80)
    print("  MAHORAGA 7C.4 — Hawkes + dual-ML layer over 6.1")
    print("=" * 80)
    print(DISCLAIMER)
    ...
    wf = run_walk_forward_h7c4(ohlcv, cfg, costs, universe_schedule)
    save_outputs_h7c4(cfg, wf, ohlcv)
    return {"cfg": cfg, "wf": wf, "ohlcv": ohlcv}


if __name__ == "__main__":
    results = run_mahoraga7c4(make_plots_flag=False, run_mode="FULL")
