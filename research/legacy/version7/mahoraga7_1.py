...
PERF-4  _precompute_overlay_context: extract all fold-invariant work from
        _custom_backtest_with_overlay into a single precompute call.
        crisis_gate, turbulence, corr_shield, score_base, rets, close, high,
        low, univ_master, rebal_set — computed once and reused across all 32
        Stage-2 iterations instead of 32 times each.
PERF-5  _build_weights_sparse: replaced the O(n_days) `for dt in idx` loop that
        wrote `w.loc[dt]` on every trading day (slow pandas scalar access) with
        a sparse approach — only iterate over rebalance dates (~260/year), store
        results in a dict, then build the full daily matrix via reindex+ffill.
        Eliminates ~4,800 redundant pandas row-writes per backtest call.
        score_rel is also cached per unique rel_tilt value so compute_scores
        is called at most once per distinct rel_tilt in the entire Stage-2 grid.
"""
...
def _precompute_overlay_context(
    ohlcv: Dict[str, pd.DataFrame],
    cfg: Mahoraga7Config,
    costs: m6.CostsConfig,
    universe_schedule: Optional[pd.DataFrame],
    ...
    """
    PERF-4: Extract all fold-invariant work from _custom_backtest_with_overlay.
    Call once per fold; pass the result to every Stage-2 iteration via `ctx`.

    Precomputes: univ_master, close/high/low/volume, idx, qqq, spy, rets,
    crisis_scale, crisis_state, turb_scale, corr_rho, corr_scale, corr_state,
    score_base, rebal_set, qqq_r, qqq_eq, vix.
    """
    ...
    if not univ_master:
        raise ValueError("[precompute_overlay_context] No valid tickers in universe")
    ...
    rebal_set  = set(close.resample(cfg.rebalance_freq).last().index.intersection(idx))
...
def _build_weights_sparse(
    ...
    """
    PERF-5: Build the daily weight matrix by iterating only over rebalance
    dates (~260/year) instead of every trading day (~5,000 total).
    Replaces the O(n_days) `for dt in idx: w.loc[dt] = ...` with a sparse
    dict → reindex → ffill, eliminating ~4,800 slow pandas scalar writes.
    """
    univ_master    = ctx["univ_master"]
    use_pit_universe = ctx["use_pit_universe"]
    close          = ctx["close"]
    rets           = ctx["rets"]
    idx            = ctx["idx"]
    score_base     = ctx["score_base"]
    rebal_set      = ctx["rebal_set"]

    rebal_weights: Dict[pd.Timestamp, np.ndarray] = {}
    last_w = np.zeros(len(univ_master), dtype=float)

    for dt in sorted(rebal_set):
        if dt not in idx:
            continue
        ...
        if not names:
            ...
            last_w = np.zeros(len(univ_master), dtype=float)
            last_w[univ_master.index(names[0])] = 1.0
        else:
            ...
            last_w = np.zeros(len(univ_master), dtype=float)
            for n in names:
                last_w[univ_master.index(n)] = float(ww.get(n, 0.0))

        rebal_weights[dt] = last_w.copy()

    # Build sparse DataFrame and forward-fill to every trading day.
    if rebal_weights:
...
def _custom_backtest_with_overlay(
    ...
    score_rel_cache: Optional[Dict[float, pd.DataFrame]] = None,
) -> Dict[str, Any]:
    """
    PERF-4: Accepts optional precomputed `ctx` from _precompute_overlay_context.
    When ctx is provided, skips crisis_gate / turbulence / corr_shield /
    compute_scores — the four most expensive operations. ~8-10x faster per call
    in Stage-2 grid iterations compared to recomputing from scratch each time.

    PERF-5: Uses _build_weights_sparse for the daily weight matrix — iterates
    only over rebalance dates and forward-fills, avoiding O(n_days) pandas
    scalar writes.

    score_rel_cache: optional dict {rel_tilt_float -> score_rel_df} so that
    compute_scores is called at most once per unique rel_tilt value.
    """
    # ── Build or reuse precomputed context ───────────────────────────────────
    if ctx is None:
        ctx = _precompute_overlay_context(ohlcv, cfg, costs, universe_schedule)
    ...
    # ── Resolve rel_tilt and build score_rel (cached per unique rel_tilt) ────
    rel_tilt_val = (
        float(daily_policy["rel_tilt"].dropna().iloc[0])
        if daily_policy["rel_tilt"].notna().any()
        else 0.60
    )
    if score_rel_cache is not None:
        if rel_tilt_val not in score_rel_cache:
            cfg_rel = _make_rel_tilt_cfg(cfg, rel_tilt_val)
            score_rel_cache[rel_tilt_val] = m6.compute_scores(close, qqq, cfg_rel)
        score_rel = score_rel_cache[rel_tilt_val]
    else:
        cfg_rel = _make_rel_tilt_cfg(cfg, rel_tilt_val)
    ...
    # ── PERF-5: sparse weight build ───────────────────────────────────────────
    w = _build_weights_sparse(ctx, cfg, universe_schedule, score_rel, action_daily)

    # ── Chandelier stops, scaling, costs ─────────────────────────────────────
...
def _calibrate_hawkes_and_overlay(
    ...
    # PERF-4: Precompute fold-invariant context once; reuse across all 32 iters.
    # PERF-5: score_rel cached per unique rel_tilt (at most 2 compute_scores calls).
    # ─────────────────────────────────────────────────────────────────────────
    fixed_hawkes = s1_best_params
    ...
    # Precompute once: crisis, turb, corr, score_base, rets, rebal_set, etc.
    ctx = _precompute_overlay_context(ohlcv, cfg_fold, costs, universe_schedule)
    # Cache score_rel per unique rel_tilt value (default grid has 2 values → 2 calls max)
    ...
    for defensive_scale, recovery_floor, rel_tilt, stress_trigger_q, recovery_trigger_q in iproduct(
        ...
        # PERF-4+5: pass precomputed ctx and score_rel_cache
...
def _run_single_fold(
    ...
    if cfg_base.variant == "7A":
        ...
        # Reuse the precomputed context built during calibration to avoid
        # recomputing crisis/turb/corr/scores a third time for the test fold.
        ctx_test = _precompute_overlay_context(ohlcv, cfg_fold, costs, universe_schedule)
        ov_bt = _custom_backtest_with_overlay(
            ohlcv, cfg_fold, costs, universe_schedule, policy_daily,
            label=f"H7B_{fold_n}", ctx=ctx_test,
        )
