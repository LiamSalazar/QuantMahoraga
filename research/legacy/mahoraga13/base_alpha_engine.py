...
from mahoraga13_utils import cross_sectional_z
...
def build_global_alpha_components(close: pd.DataFrame, qqq: pd.Series, cfg: Mahoraga13Config) -> Dict[str, pd.DataFrame]:
    idx = close.index
...
def _build_target_weights(score: pd.DataFrame, pre: Dict[str, Any], cfg: Mahoraga13Config) -> pd.DataFrame:
    idx = pre["idx"]
    close = pre["close"]
    rets = pre["rets"]
