...
- Corrected 3 print labels from "7C.2" to "7C.3" in _run_single_fold.
...
DISCLAIMER = r"""
═══════════════════════════════════════════════════════════════════════════════
  MAHORAGA 7C.3 — HAWKES + DUAL-ML LAYER OVER MAHORAGA 6.1
───────────────────────────────────────────────────────────────────────────────
...
class Mahoraga7C3Config(h7.Mahoraga7Config):
    variant: str = "7C.3"
    outputs_dir: str = "mahoraga7c3_outputs"
    plots_dir: str = "mahoraga7c3_plots"
    label: str = "MAHORAGA_7C3"
    ...
    score_w_recovery_hit: float = 0.16
    score_w_recovery_fire: float = 0.10
    score_pen_intervention: float = 0.10
    score_pen_recovery_excess: float = 0.01
    score_pen_recovery_miss: float = 0.14
...
def _precompute_fixed_thresholds(
    ...
    """
    PERF-7C2: Precompute the 8 thresholds that depend only on fixed cfg constants.
    Call once per outer pair; reuse across all 320 policy calls in Phase A + B.
    Only stress_thr / rec_thr are excluded — they depend on grid params (stq, rtq).
    """
...
def _policy_from_dual_probs_vec(
    ...
    Identical 7C.3 logic (5-condition recovery gate, 4-branch allow_recover,
    4-component rec_scale with momentum_relief, recover_cond_count output).
    Zero Python-level iteration over dates.
    """
    idx = weekly_df.index

    # ── Arrays (one extraction pass, no per-row .at[] access) ────────────────
    ...
    # ── Vectorized conditions (identical to original per-row logic) ───────────
    chop = (
    ...
    cond_count  = (cond_rec.astype(int) + cond_spread.astype(int) + cond_scale.astype(int)
                   + cond_breadth.astype(int) + cond_rel.astype(int))

    recover_opp = (pr >= recovery_prob_trigger) & ~chop & (cond_count >= cfg.recovery_min_conditions)
    can_frag    = (pf >= prob_trigger) & ~(chop & ~crisis_on)

    # 4-branch allow_recover
    ...
    # Fragility sub-actions
    rel_confirm    = (rel_top >= rel_thr_f) & (rm_mom > 0) & (rm_tr > 0)
    ...
    # 4-component rec_scale (with momentum_relief — unique to 7C.3)
    denom_prob    = max(1e-6, 1.0 - recovery_prob_trigger)
    denom_spread  = max(1e-6, max(abs(spread_pos_thr), 1.0))
    denom_scale   = max(1e-6, max(scale_relaxed_thr, 1.0))   # uses scale_relaxed_thr
    ...
    # Reduction when neither crisis_on nor rec_prob_hi (0.95, not 0.90 as in 7C.2)
...
def _run_single_fold(
    ...
    print(f"\n  ── 7C.3 FOLD {fold_n}/{total_folds} ──")
    cfg_fold = _get_fold_cfg(ohlcv, cfg_base, costs, universe_schedule, baseline_row)
    print(f"  [fold {fold_n}] Calibrating 7C.3 on train via inner validation …")
    ...
    ov_bt = h7._custom_backtest_with_overlay(ohlcv, cfg_fold, costs, universe_schedule, policy_daily, label=f"H7C3_{fold_n}", ctx=ctx_test, score_rel_cache={})
    ...
    so = m6.summarize(ro, qo, eo, to, cfg_fold, f"H7C3_{fold_n}")
    ...
    print(f"  [fold {fold_n}] BASE Sharpe={sb['Sharpe']:.3f} | 7C.3 Sharpe={so['Sharpe']:.3f} | Δ={so['Sharpe'] - sb['Sharpe']:+.3f}")
    ...
    return {
        ...
        "fold_row": {
            ...
            "H7C3_CAGR%": round(so["CAGR"] * 100, 2),
            "H7C3_Sharpe": round(so["Sharpe"], 4),
            "H7C3_MaxDD%": round(so["MaxDD"] * 100, 2),
...
def _selection_audit(wf: Dict[str, Any], cfg: Mahoraga7C3Config) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        ...
        {
            "Method": "H7C3",
            "MeanSharpe": fr["H7C3_Sharpe"].mean(),
            "MeanCAGR%": fr["H7C3_CAGR%"].mean(),
            "MeanMaxDD%": fr["H7C3_MaxDD%"].mean(),
...
def _final_report_text(cfg: Mahoraga7C3Config, wf: Dict[str, Any], ohlcv: Dict[str, pd.DataFrame]) -> str:
    base_sum = m6.summarize(wf["base_oos_r"], wf["base_oos_eq"], wf["base_oos_exp"], wf["base_oos_to"], cfg, "BASE_OOS")
    ov_sum = m6.summarize(wf["ov_oos_r"], wf["ov_oos_eq"], wf["ov_oos_exp"], wf["ov_oos_to"], cfg, "H7C3_OOS")
    sel = _selection_audit(wf, cfg)
    reg = _regime_comparison(wf["base_oos_r"], wf["ov_oos_r"], cfg, ohlcv)
    fr = wf["fold_results"]
    lines = []
    lines.append("MAHORAGA 7C.3 — FINAL REPORT")
    lines.append("=" * 78)
    lines.append(DISCLAIMER)
    lines.append("\nOOS COMPARISON")
    lines.append(f"  BASELINE CAGR={base_sum['CAGR']*100:.2f}%  Sharpe={base_sum['Sharpe']:.3f}  MaxDD={base_sum['MaxDD']*100:.2f}%")
    lines.append(f"  H7C3    CAGR={ov_sum['CAGR']*100:.2f}%  Sharpe={ov_sum['Sharpe']:.3f}  MaxDD={ov_sum['MaxDD']*100:.2f}%")
    lines.append("\nSELECTION AUDIT")
...
def save_outputs_h7c3(cfg: Mahoraga7C3Config, wf: Dict[str, Any], ohlcv: Dict[str, pd.DataFrame]):
    d = cfg.outputs_dir
    _ensure_dir(d)
    base_sum = m6.summarize(wf["base_oos_r"], wf["base_oos_eq"], wf["base_oos_exp"], wf["base_oos_to"], cfg, "BASE_OOS")
    ov_sum = m6.summarize(wf["ov_oos_r"], wf["ov_oos_eq"], wf["ov_oos_exp"], wf["ov_oos_to"], cfg, "H7C3_OOS")
    ...
    wf["fold_results"].to_csv(os.path.join(d, "walk_forward_folds_7c3.csv"), index=False)
    ...
    print(f"\n  [outputs → ./{d}/]")
    print("    comparison_oos.csv, comparison_full.csv, walk_forward_folds_7c3.csv")
...
def run_mahoraga7c3(make_plots_flag: bool = False, run_mode: str = "FULL") -> Dict[str, Any]:
    if not _SKLEARN:
        raise RuntimeError("scikit-learn is required for Mahoraga 7C")
    print("=" * 80)
    print("  MAHORAGA 7C.3 — Hawkes + dual-ML layer over 6.1")
    print("=" * 80)
    print(DISCLAIMER)
    ...
    wf = run_walk_forward_h7c3(ohlcv, cfg, costs, universe_schedule)
    save_outputs_h7c3(cfg, wf, ohlcv)
    return {"cfg": cfg, "wf": wf, "ohlcv": ohlcv}


if __name__ == "__main__":
    results = run_mahoraga7c3(make_plots_flag=False, run_mode="FULL")
