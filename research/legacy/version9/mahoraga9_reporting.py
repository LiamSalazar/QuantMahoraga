...
def save_outputs_v9(cfg: Mahoraga9Config, wf: Dict[str, Any]) -> Dict[str, Path]:
    ...
    policy_rows = pd.DataFrame([{"fold": r["fold"], **r["best_policy"]} for r in wf["results"]])
    ...
    base_s = _summary_from_stitched(wf["stitched_base"], cfg, "BASELINE_6_1_COMPAT")
    v9_s = _summary_from_stitched(wf["stitched_v9"], cfg, "V9")

    report_name = "final_report_fast.txt" if cfg.run_mode.upper() == "FAST" else "final_report.txt"
    report_path = out_dir / report_name
    with report_path.open("w", encoding="utf-8") as f:
        f.write("MAHORAGA 9 — FINAL REPORT\n")
        f.write("=" * 78 + "\n\n")
        f.write("OOS COMPARISON\n")
        f.write(
            f"  BASELINE CAGR={base_s['CAGR']*100:.2f}%  Sharpe={base_s['Sharpe']:.3f}  MaxDD={base_s['MaxDD']*100:.2f}%\\n"
        )
        f.write(
            f"  V9       CAGR={v9_s['CAGR']*100:.2f}%  Sharpe={v9_s['Sharpe']:.3f}  MaxDD={v9_s['MaxDD']*100:.2f}%\\n\\n"
        )
        f.write("FOLD SUMMARY\n")
