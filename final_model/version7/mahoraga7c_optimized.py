...
def _calibrate_7c(
    ...
    # PERF-7C: precompute X_val once — it never changes within this fold.
    # X_val does not depend on horizon_weeks, frag_q, or any policy param.
    if len(inner_val_idx) >= 5:
        X_val_raw = ml_feat_full.loc[inner_val_idx, FEATURE_COLS].copy()
    else:
        X_val_raw = None
    ...
    # PERF-7C: outer loop over label-defining params only (4 combos).
    # The RF is re-fitted exactly once per (horizon_weeks, frag_q) pair.
    # All policy params are swept in the inner loop using cached probs_val.
    for horizon_weeks, frag_q in iproduct(
        cfg_fold.fragility_horizon_weeks_grid,
        cfg_fold.fragility_quantile_grid,
    ):
        if len(inner_train_idx) < cfg_fold.min_train_weeks or len(inner_val_idx) < 5:
            continue

        # Utility is cached per horizon_weeks (2 calls max across entire Stage-2)
        if horizon_weeks not in utility_cache:
            utility_cache[horizon_weeks] = _future_window_utility(
                base_bt["returns_net"], base_bt["equity"], wk_idx,
                horizon_weeks, cfg_fold.utility_dd_penalty
            )
        utility = utility_cache[horizon_weeks]

        # Labels depend only on (horizon_weeks, frag_q) — compute once per pair
        thr_utility = float(np.nanquantile(
            utility.loc[inner_train_idx].dropna(), frag_q
        )) if utility.loc[inner_train_idx].dropna().size else np.nan
        y = (utility <= thr_utility).astype(float)

        X_train, y_train, med = _prepare_xy(
            ml_feat_full.loc[inner_train_idx], y.loc[inner_train_idx]
        )

        # Single RF fit for this (horizon_weeks, frag_q) pair
        model = _fit_fragility_model(X_train, y_train, cfg_fold)
        if model is None:
            continue

        # Predict probabilities on val once — reused across all 64 policy combos
        X_val = X_val_raw.fillna(med).replace([np.inf, -np.inf], np.nan).fillna(med)
        probs_val = pd.Series(
            model.predict_proba(X_val)[:, 1], index=inner_val_idx
        )

        # Inner loop: policy params only (64 combos) — no re-fitting
        for prob_trigger, defensive_scale, recovery_floor, rel_tilt, stress_trigger_q, recovery_trigger_q in iproduct(
            ...
            policy_weekly = _policy_from_fragility_probs(
                ml_feat_full.loc[inner_val_idx], probs_val,
