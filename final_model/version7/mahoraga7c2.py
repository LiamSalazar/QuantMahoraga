...
Performance fix (PERF-7C2)
---------------------------
`_policy_from_dual_probs` was a Python for-loop over weekly dates (~260 iter/call),
called 320 times per fold (Phase A: 64, Phase B: 256). Total ~83,200 Python iterations.

Additionally each call recomputed 8 quantile thresholds from train_weekly_ref, of which
6 depend only on fixed cfg constants (never change within a fold's grid sweep) and only
2 depend on grid params (stress_trigger_q, recovery_trigger_q — 4 unique pairs).

Fixed in three steps:
1. `_precompute_fixed_thresholds`: compute the 6 cfg-constant thresholds once per
   outer pair (not 320 times). Cache (stress_thr, rec_thr) for the 4 (stq, rtq) pairs.
2. `_policy_from_dual_probs_vec`: fully vectorized replacement using numpy arrays
   instead of a Python date loop. Identical logic, zero Python-level iteration.
3. Remove dead `for ... pass` loop (leftover from 7C.1 transition).
...
DISCLAIMER = r"""
═══════════════════════════════════════════════════════════════════════════════
  MAHORAGA 7C.2 — HAWKES + DUAL-ML LAYER OVER MAHORAGA 6.1
───────────────────────────────────────────────────────────────────────────────
...
@dataclass
class Mahoraga7C2Config(h7.Mahoraga7Config):
    variant: str = "7C.2"
    outputs_dir: str = "mahoraga7c2_outputs"
    plots_dir: str = "mahoraga7c2_plots"
    label: str = "MAHORAGA_7C2"
    ...
    target_recovery_rate: float = 0.08
    target_recovery_hit_rate: float = 0.35
    recovery_prob_trigger: float = 0.55
    recovery_return_quantile: float = 0.80
    recovery_low_scale_quantile: float = 0.45
    recovery_spread_quantile: float = 0.55
    recovery_breadth_quantile: float = 0.55
    recovery_floor_cap: float = 0.70
    recovery_prob_boost: float = 0.18
    recovery_spread_boost: float = 0.10
    recovery_scale_relief_boost: float = 0.12
    recovery_extra_prob_margin: float = 0.06
    ...
    score_w_sharpe: float = 0.50
    score_w_dd: float = 0.20
    score_w_cagr: float = 0.12
    score_w_recovery_hit: float = 0.08
    score_w_recovery_fire: float = 0.04
    score_pen_intervention: float = 0.10
    score_pen_recovery_excess: float = 0.02
    score_pen_recovery_miss: float = 0.08
...
def _load_baseline_folds(cfg: Mahoraga7C2Config) -> pd.DataFrame:
...
def _build_ml_feature_table(hawkes_df: pd.DataFrame, base_bt: Dict[str, Any], cfg: Mahoraga7C2Config) -> pd.DataFrame:
...
def _fit_binary_model(X: pd.DataFrame, y: pd.Series, cfg: Mahoraga7C2Config):
    if not _SKLEARN:
        return None
    vc = y.value_counts()
...
def _precompute_fixed_thresholds(
    ...
    """
    PERF-7C2: Precompute the 6 thresholds that depend only on fixed cfg constants
    (not on any grid parameter). Call once per outer pair; pass to the vectorized
    policy function instead of recomputing inside every call.

    Only stress_thr and rec_thr are excluded — they depend on stress_trigger_q /
    recovery_trigger_q which are grid params. Cache those as a (stq, rtq) → (s, r)
    dict separately.
    """
...
def _policy_from_dual_probs_vec(
    ...
    """
    PERF-7C2: Fully vectorized replacement for the Python date-loop policy function.
    Identical logic, zero Python-level iteration over dates.

    Accepts precomputed stress_thr / rec_thr (grid-varying) and fixed_thr dict
    (fold-level constants from _precompute_fixed_thresholds).
    """
    idx = weekly_df.index
    n   = len(idx)

    # ── Extract columns as numpy arrays (one pass, no per-row access) ────────
    ...
    # ── Unpack precomputed thresholds ─────────────────────────────────────────
    rel_thr        = fixed_thr["rel_thr"]
    ...
    # ── Vectorized conditions (identical logic to original) ───────────────────
    chop = (
    ...
    recover_opp = (
        (pr      >= recovery_prob_trigger)
      & (rec     >= rec_thr)
      & (spread  >= spread_pos_thr)
      & (bscale  <= scale_low_thr)
      & (breadth >= breadth_rec_thr)
      & (rel_top >= rel_thr)
      & (qqq21   >= -chop_ret_thr)
      & (~chop)
    )
    ...
    allow_recover = recover_opp & (
        crisis_on
      | rec_prob_hi
      | ((stress < rec) & (spread >= spread_pos_thr))
    )

    # Fragility sub-actions (only matter when can_frag and not allow_recover)
    rel_confirm    = (rel_top >= rel_thr) & (rm_mom > 0) & (rm_tr > 0)
    stress_dominant= (stress >= stress_thr) & ((stress > rec) | (spread < spread_pos_thr))

    frag_action = np.where(stress_dominant, "DEFENSIVE_LIGHT",
                  np.where(rel_confirm, "REL_TILT", "DEFENSIVE_LIGHT"))

    action = np.where(allow_recover, "RECOVERY_OVERRIDE",
             np.where(can_frag, frag_action, "BASELINE"))

    # ── Scalar outputs ────────────────────────────────────────────────────────
    ext_scale_arr = np.where(action == "DEFENSIVE_LIGHT", defensive_scale, 1.0)

    # rec_scale: adaptive formula, only non-zero for RECOVERY_OVERRIDE
    denom_prob   = max(1e-6, 1.0 - recovery_prob_trigger)
    denom_spread = max(1e-6, max(abs(spread_pos_thr), 1.0))
    denom_scale  = max(1e-6, max(scale_low_thr, 1.0))
    ...
    scale_gap     = np.clip(scale_low_thr - bscale,       0.0, None) / denom_scale
    ...
    # Apply 0.90 reduction when neither crisis_on nor rec_prob_hi
    reduce = np.where(~crisis_on & ~rec_prob_hi, 0.90, 1.0)
    rec_scale_raw = rec_scale_raw * reduce
    rec_scale_arr = np.where(
        action == "RECOVERY_OVERRIDE",
        np.clip(rec_scale_raw, recovery_floor, cfg.recovery_floor_cap),
        0.0,
    )
...
def _policy_from_dual_probs(
    ...
    """
    Thin wrapper retained for _run_single_fold (test phase), where the function
    is called only once and precomputed thresholds are not cached externally.
...
# ─────────────────────────────────────────────────────────────────────────────
# PERF-7C2: Weight/chandelier/vol precomputation helpers
# ─────────────────────────────────────────────────────────────────────────────
...
def _score_from_weights(
    ...
    """
    PERF-7C2 Phase B: apply scaling and score given precomputed w_exec_1x + vol_sc.
    Only vectorized pandas/numpy ops — no loops, no chandelier, no weight build.
    Returns (score, ov_sum, intervention_rate, recovery_rate, recovery_opp_rate, recovery_hit_rate).
    ...
    score = _score_overlay(base_sum, ov_sum, intervention_rate, recovery_rate, recovery_opp_rate, recovery_hit_rate, cfg)
    return score, ov_sum, intervention_rate, recovery_rate, recovery_opp_rate, recovery_hit_rate
...
def _calibrate_7c(
    ...
    for horizon_weeks, frag_q in iproduct(cfg_fold.fragility_horizon_weeks_grid, cfg_fold.fragility_quantile_grid):
        ...
        # ── PERF-7C2: precompute the 6 fixed thresholds once per outer pair ──
        # (They depend only on cfg constants, not on any grid parameter.)
        fixed_thr = _precompute_fixed_thresholds(ml_feat_full.loc[inner_train_idx], cfg_fold)

        # Cache (stress_thr, rec_thr) for the 4 unique (stq, rtq) pairs.
        # Only these 2 thresholds vary with grid params.
        ...
        # (prob_trigger, stress_trigger_q, recovery_trigger_q, rel_tilt).
        # defensive_scale and recovery_floor do NOT affect action → same weights.
        # Uses vectorized policy function — no Python date loop.
        ...
        for pt, stq, rtq, rlt in iproduct(
            ...
            # PERF-7C2: vectorized, no Python date loop
        ...
        # Uses vectorized policy function — no Python date loop per combo.
        for prob_trigger, defensive_scale, recovery_floor, rel_tilt, stress_trigger_q, recovery_trigger_q in iproduct(
            ...
            # PERF-7C2: vectorized, uses precomputed fixed_thr and cached stress/rec thresholds
            ...
            score, ov_sum, intervention_rate, recovery_rate, recovery_opp_rate, recovery_hit_rate = _score_from_weights(
...
def _run_single_fold(
    ...
    print(f"\n  ── 7C.2 FOLD {fold_n}/{total_folds} ──")
    cfg_fold = _get_fold_cfg(ohlcv, cfg_base, costs, universe_schedule, baseline_row)
    print(f"  [fold {fold_n}] Calibrating 7C.2 on train via inner validation …")
    ...
    ov_bt = h7._custom_backtest_with_overlay(ohlcv, cfg_fold, costs, universe_schedule, policy_daily, label=f"H7C2_{fold_n}", ctx=ctx_test, score_rel_cache={})
    ...
    so = m6.summarize(ro, qo, eo, to, cfg_fold, f"H7C2_{fold_n}")
    ...
    print(f"  [fold {fold_n}] BASE Sharpe={sb['Sharpe']:.3f} | 7C.2 Sharpe={so['Sharpe']:.3f} | Δ={so['Sharpe'] - sb['Sharpe']:+.3f}")
    ...
    return {
        ...
        "fold_row": {
            ...
            "H7C2_CAGR%": round(so["CAGR"] * 100, 2),
            "H7C2_Sharpe": round(so["Sharpe"], 4),
            "H7C2_MaxDD%": round(so["MaxDD"] * 100, 2),
...
def _selection_audit(wf: Dict[str, Any], cfg: Mahoraga7C2Config) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        ...
        {
            "Method": "H7C2",
            "MeanSharpe": fr["H7C2_Sharpe"].mean(),
            "MeanCAGR%": fr["H7C2_CAGR%"].mean(),
            "MeanMaxDD%": fr["H7C2_MaxDD%"].mean(),
...
def _regime_comparison(base_r: pd.Series, ov_r: pd.Series, cfg: Mahoraga7C2Config, ohlcv: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    return h7._regime_comparison(base_r, ov_r, cfg, ohlcv)


def _final_report_text(cfg: Mahoraga7C2Config, wf: Dict[str, Any], ohlcv: Dict[str, pd.DataFrame]) -> str:
    base_sum = m6.summarize(wf["base_oos_r"], wf["base_oos_eq"], wf["base_oos_exp"], wf["base_oos_to"], cfg, "BASE_OOS")
    ov_sum = m6.summarize(wf["ov_oos_r"], wf["ov_oos_eq"], wf["ov_oos_exp"], wf["ov_oos_to"], cfg, "H7C2_OOS")
    sel = _selection_audit(wf, cfg)
    reg = _regime_comparison(wf["base_oos_r"], wf["ov_oos_r"], cfg, ohlcv)
    fr = wf["fold_results"]
    lines = []
    lines.append("MAHORAGA 7C.2 — FINAL REPORT")
    lines.append("=" * 78)
    lines.append(DISCLAIMER)
    lines.append("\nOOS COMPARISON")
    lines.append(f"  BASELINE CAGR={base_sum['CAGR']*100:.2f}%  Sharpe={base_sum['Sharpe']:.3f}  MaxDD={base_sum['MaxDD']*100:.2f}%")
    lines.append(f"  H7C2    CAGR={ov_sum['CAGR']*100:.2f}%  Sharpe={ov_sum['Sharpe']:.3f}  MaxDD={ov_sum['MaxDD']*100:.2f}%")
    lines.append("\nSELECTION AUDIT")
...
def save_outputs_h7c2(cfg: Mahoraga7C2Config, wf: Dict[str, Any], ohlcv: Dict[str, pd.DataFrame]):
    d = cfg.outputs_dir
    _ensure_dir(d)
    base_sum = m6.summarize(wf["base_oos_r"], wf["base_oos_eq"], wf["base_oos_exp"], wf["base_oos_to"], cfg, "BASE_OOS")
    ov_sum = m6.summarize(wf["ov_oos_r"], wf["ov_oos_eq"], wf["ov_oos_exp"], wf["ov_oos_to"], cfg, "H7C2_OOS")
    ...
    wf["fold_results"].to_csv(os.path.join(d, "walk_forward_folds_7c2.csv"), index=False)
    ...
    print(f"\n  [outputs → ./{d}/]")
    print("    comparison_oos.csv, comparison_full.csv, walk_forward_folds_7c2.csv")
...
def run_mahoraga7c2(make_plots_flag: bool = False, run_mode: str = "FULL") -> Dict[str, Any]:
    if not _SKLEARN:
        raise RuntimeError("scikit-learn is required for Mahoraga 7C")
    print("=" * 80)
    print("  MAHORAGA 7C.2 — Hawkes + dual-ML layer over 6.1")
    print("=" * 80)
    print(DISCLAIMER)
    ...
    wf = run_walk_forward_h7c2(ohlcv, cfg, costs, universe_schedule)
    save_outputs_h7c2(cfg, wf, ohlcv)
    return {"cfg": cfg, "wf": wf, "ohlcv": ohlcv}


if __name__ == "__main__":
    results = run_mahoraga7c2(make_plots_flag=False, run_mode="FULL")
