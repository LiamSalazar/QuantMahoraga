...
class Mahoraga14Config(m6.Mahoraga6Config):
    variant: str = "14.2"
    label: str = "MAHORAGA_14_2"
    cache_dir: str = field(default_factory=lambda: str(_project_root() / "data_cache"))
    outputs_dir: str = field(default_factory=lambda: str(_project_root() / "mahoraga14_2_outputs"))
    plots_dir: str = field(default_factory=lambda: str(_project_root() / "mahoraga14_2_plots"))
    official_baseline_label: str = "BASE_ALPHA_V2"
    historical_benchmark_label: str = "LEGACY"
    model_label: str = "M14_2_LONG_PARTICIPATION"
    main_variant_key: str = "STRUCTURAL_DEFENSE_ONLY"
    continuation_variant_key: str = "CONTINUATION_PRESSURE_V2_ONLY"
    combo_variant_key: str = "MAHORAGA14_2_LONG_PARTICIPATION"
    full_primary_variant_key: str = "MAHORAGA14_2_LONG_PARTICIPATION"
    primary_variant_key: str = "MAHORAGA14_2_LONG_PARTICIPATION"
    ...
    participation_long_budget_floor: float = 0.55
    participation_long_budget_base: float = 0.78
    participation_long_budget_ceiling: float = 1.00
    participation_gate_max: float = 1.22
    participation_vol_mult_max: float = 1.18
    participation_exp_cap_max: float = 1.28
    participation_leader_blend_max: float = 0.45
    participation_mix_lift: float = 0.14
    participation_beta_penalty_mult: float = 0.15
    participation_raw_rel_boost_lift: float = 0.22
    participation_cash_redeploy_scale_cap: float = 1.65
    participation_max_name_weight: float = 0.72
    participation_redeploy_headroom_floor: float = 0.04
    risk_backoff_budget_floor: float = 0.45
    risk_backoff_hard_budget: float = 0.28
    risk_backoff_gate_floor: float = 0.88
    risk_backoff_vol_floor: float = 0.90
    risk_backoff_exp_floor: float = 0.82
    risk_backoff_hard_break_risk: float = 0.72
    risk_backoff_hard_fragility: float = 0.72
    risk_backoff_hard_benchmark_weakness: float = 0.70
    ...
    diagnostics_bull_weaker_mult: float = 0.75
    diagnostics_bull_stronger_mult: float = 1.25
