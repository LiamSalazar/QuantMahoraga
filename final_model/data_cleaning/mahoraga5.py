...
def backtest(
    ...
    cap = (crisis_scale * turb_scale * ext_scale).clip(0.0, cfg.max_exposure)
    tgt_sc = pd.Series(np.minimum(vol_sc.values, cap.values), index=idx).clip(0.0, cfg.max_exposure)
    exec_sc = tgt_sc.shift(1).fillna(0.0)
    w_exec = w_exec_1x.mul(exec_sc, axis=0)
    to, tc = _costs(w_exec, costs)
...
def run_walk_forward(
    ...
    trading_idx = pd.DatetimeIndex(ohlcv["close"].index)
    folds = build_contiguous_folds(cfg_base, trading_idx)
    all_oos_r, fold_results, all_sweeps = [], [], []
    last_best = None
    fold_runtime = []
    ...
    return oos_r, oos_eq, fold_results, pd.concat(all_sweeps, ignore_index=True), oos_label, selected_config_info
...
if __name__ == "__main__":
    results = run_mahoraga5(make_plots_flag=True, run_robustness=True)
