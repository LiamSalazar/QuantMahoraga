...
def build_allocator_controls(state: pd.DataFrame, cfg: Mahoraga15AConfig) -> pd.DataFrame:
    ...
    out["directional_bear"] = clip01(0.45 * state["benchmark_weakness"] + 0.35 * state["bear_persistence"] + 0.20 * state["transition_shock"])
    out["crisis_activation"] = clip01(state["crisis_activation"])
    out["crisis_transition"] = clip01(0.60 * state["transition_shock"] + 0.25 * state["hawkes_stress"] + 0.15 * state["break_risk"])
    out["crisis_persistence"] = clip01(0.55 * state["bear_persistence"] + 0.25 * state["drawdown_pressure"] + 0.20 * state["benchmark_weakness"])

    beta_target_mix = clip01(0.55 * out["crisis_activation"] + 0.30 * out["crisis_persistence"] + 0.15 * out["directional_bear"])
    ...
    long_de_risk_pressure = clip01(
        0.45 * out["crisis_activation"]
        + 0.20 * state["realized_vol_pressure"]
        + 0.15 * state["drawdown_pressure"]
        + 0.10 * out["beta_gap_score"]
        + 0.10 * out["crisis_transition"]
    )
    ...
    short_regime_mix = clip01(
        0.45 * out["crisis_activation"]
        + 0.25 * out["crisis_transition"]
        + 0.15 * out["beta_gap_score"]
        + 0.15 * state["break_risk"]
    )
    ...
    out["short_cap_dynamic"] = np.maximum(
        out["short_cap_dynamic"],
        cfg.allocator_crisis_overlay_floor * out["crisis_activation"] * cfg.allocator_crisis_overlay_cap_weight,
    )
    out["short_cap_dynamic"] = np.minimum(out["short_cap_dynamic"], out["short_cap_regime_band"])

    out["cash_target"] = (
        cfg.allocator_cash_floor
        + 0.04 * out["crisis_activation"]
        + 0.03 * state["realized_vol_pressure"]
        + 0.02 * state["break_risk"]
        - 0.02 * state["continuation_relief"]
    ).clip(cfg.allocator_cash_floor, cfg.allocator_cash_target_ceiling)

    out["net_exposure_floor_dynamic"] = np.select(
        [out["crisis_activation"] < 0.35, out["crisis_activation"] < 0.75],
    ...
    out["net_exposure_floor_dynamic"] = np.minimum(
        ...
        cfg.allocator_net_exposure_floor_benign - 0.40 * out["crisis_activation"],
    )

    out["reaction_multiplier"] = (
        cfg.allocator_speed_floor
        + (cfg.allocator_speed_ceiling - cfg.allocator_speed_floor)
        * clip01(0.55 * out["crisis_transition"] + 0.25 * state["hawkes_stress"] + 0.20 * state["break_risk"])
    ).clip(cfg.allocator_speed_floor, cfg.allocator_speed_ceiling)
    out["short_up_speed"] = (
...
def finalize_allocator_trace(
    ...
    for dt, row in out.iterrows():
        ...
        reaction_multiplier = float(row["reaction_multiplier"])
        ...
        error = raw_short - prev_short
        if error >= 0.0:
            base_step = float(row["short_up_speed"]) * reaction_multiplier * error
            shock_kick = cfg.allocator_short_shock_kick * crisis_transition * max(raw_short, prev_short)
            desired_step = base_step + shock_kick
        else:
            release_speed = float(row["short_down_speed"]) / max(1.0, reaction_multiplier)
            desired_step = release_speed * error
            if crisis_activation < cfg.hedge_crisis_onset:
                desired_step -= (1.0 - crisis_activation) * (1.0 + continuation_relief) * cfg.allocator_short_release_decay * prev_short

        short_velocity = cfg.allocator_short_velocity_decay * prev_short_velocity + desired_step
        short_velocity = float(np.clip(short_velocity, -cfg.allocator_short_step_cap, cfg.allocator_short_step_cap))
        short_target = max(0.0, prev_short + short_velocity)
        ...
        max_short_by_net = max(0.0, long_budget - float(row["net_exposure_floor_dynamic"]))
        ...
        final_rows.append(
            {
                ...
                "crisis_short_budget": short_budget,
                ...
                "short_speed_applied": float(row["short_up_speed"] if error >= 0.0 else row["short_down_speed"]),
                "reaction_multiplier_applied": reaction_multiplier,
                "long_step_applied": long_mult_step,
                "short_step_applied": short_velocity,
                "short_velocity_state": short_velocity,
            }
        )
        prev_short = short_budget
        prev_short_velocity = short_velocity
