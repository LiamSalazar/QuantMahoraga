...
def build_hawkes_features(weekly_df: pd.DataFrame, decay: float, cfg: Mahoraga11Config) -> pd.DataFrame:
    ...
    recovery_event = (
        ...
        & (weekly_df["qqq_drawdown"] > weekly_df["qqq_drawdown"].shift(1).bfill())
    ).astype(float)
