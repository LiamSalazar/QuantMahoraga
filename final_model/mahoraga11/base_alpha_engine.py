...
def build_alpha_components(close: pd.DataFrame, qqq: pd.Series, cfg: Mahoraga11Config) -> Dict[str, pd.DataFrame]:
    ...
    legacy_core = cross_sectional_z(m6.compute_scores(close, qqq, cfg))
...
def build_engine_score(
    ...
    legacy_core = components["legacy_core"]
    ...
    adaptive_score = 0.65 * raw_score + 0.35 * resid_score - beta_drag
...
def _build_target_weights(score: pd.DataFrame, pre: Dict[str, Any], cfg: Mahoraga11Config) -> pd.DataFrame:
    idx = pre["idx"]
    close = pre["close"]
    rets = pre["rets"]
