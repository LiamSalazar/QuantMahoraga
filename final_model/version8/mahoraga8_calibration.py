...
def _score_candidate(base_sum: Dict[str, float], ov_sum: Dict[str, float], panic_sharpe: float, stress_sharpe: float, missed_rebound: float, recovery_capture_rate: float, turnover_ann: float, worst_fold_proxy: float, cfg: Mahoraga8Config) -> float:
...
def _calibrate_stage1_hawkes(feat_full: pd.DataFrame, train_start: str, inner_train_end: str, inner_val_start: str, inner_val_end: str, cfg_fold: Mahoraga8Config, crisis_state_weekly: pd.Series) -> Tuple[Dict[str, float], pd.DataFrame]:
    ...
    for stress_q, recovery_q, decay, stress_scale, recovery_scale in iproduct(overrides['stress_q_grid'], overrides['recovery_q_grid'], overrides['hawkes_decay_grid'], overrides['stress_scale_grid'], overrides['recovery_scale_grid']):
        hawkes_df, _ = h7._build_hawkes_signals(feat_full, stress_q, recovery_q, decay, stress_scale, recovery_scale, feat_full.loc[train_start:inner_train_end])
        score = h7._diagnostic_alignment_score(hawkes_df.loc[inner_val_start:inner_val_end], crisis_state_weekly)
        row = {'stress_q': stress_q, 'recovery_q': recovery_q, 'decay': decay, 'stress_scale': stress_scale, 'recovery_scale': recovery_scale, 'score': score}
        rows.append(row)
        if score > best_score:
            best_score = score
            best = row.copy()
