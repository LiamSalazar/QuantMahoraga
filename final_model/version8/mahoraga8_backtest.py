...
def _get_fold_cfg(ohlcv: Dict[str, pd.DataFrame], base_cfg: Mahoraga8Config, costs: m6.CostsConfig, universe_schedule: Optional[pd.DataFrame], fold_row: pd.Series) -> Mahoraga8Config:
    ...
    final_train_tickers = m6.get_training_universe(train_end, universe_schedule, cfg.universe_static, list(ohlcv['close'].columns))
...
def _run_single_fold_h8(fold_n: int, baseline_row: pd.Series, ohlcv: Dict[str, pd.DataFrame], cfg_base: Mahoraga8Config, costs: m6.CostsConfig, universe_schedule: Optional[pd.DataFrame]) -> Dict[str, Any]:
