...
def _normalize_universe_schedule(universe_schedule: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
...
def run_walk_forward_h8(ohlcv: Dict[str, Any], cfg: Mahoraga8Config, costs: m6.CostsConfig, universe_schedule: Optional[Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "stitched_h8": stitch_oos_path(results, "ov_bt"),
    }


def run_mahoraga8(make_plots_flag: bool = False, run_mode: str = "FAST") -> Dict[str, Any]:
    print("=" * 80)
    print("  MAHORAGA 8.2 HM — Hawkes + Markov regime fusion")
    print("=" * 80)
    cfg = Mahoraga8Config()
