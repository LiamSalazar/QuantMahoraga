...
def _normalize_universe_schedule(universe_schedule: Optional[Any]) -> Optional[Any]:
    if universe_schedule is None:
        return universe_schedule
    if isinstance(universe_schedule, pd.DataFrame) and len(universe_schedule):
        if not isinstance(universe_schedule.index, pd.DatetimeIndex):
            for c in ['date', 'as_of', 'rebalance_date', 'effective_date', 'timestamp']:
                if c in universe_schedule.columns:
                    universe_schedule[c] = pd.to_datetime(universe_schedule[c], errors='coerce')
                    universe_schedule = universe_schedule.dropna(subset=[c]).sort_values(c).set_index(c)
                    break
    return universe_schedule
...
def run_mahoraga8(make_plots_flag: bool = False, run_mode: str = 'FAST') -> Dict[str, Any]:
    print('=' * 80)
    print('  MAHORAGA 8.1 LITE — Integrated risk-adaptive core')
    print('=' * 80)
    cfg = Mahoraga8Config()
    ...
    print('[2] Canonical universe engine …')
    ...
    universe_schedule = _normalize_universe_schedule(universe_schedule)
    print(f"  [universe] {len(universe_schedule)} reconstitution dates built")
    print(f"[3] Walk-forward {cfg.variant} …")
