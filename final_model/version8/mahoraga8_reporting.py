...
def build_regime_comparison(wf: Dict[str, Any], cfg: Mahoraga8Config) -> pd.DataFrame:
    ...
    for reg in ["NORMAL", "STRESS", "PANIC", "RECOVERY"]:
        ...
        so = m6.summarize(ro, ov_eq, pd.Series(1.0, index=ro.index), pd.Series(0.0, index=ro.index), cfg, f"H8_2HM_{reg}")
        rows.append({
            "Regime": reg,
            "Days": len(rb),
    ...
    return pd.DataFrame([
        {
            ...
            "MeanSharpe": round(float(fold_df["BASE_Sharpe"].mean()), 5),
            "MeanCAGR%": round(float(fold_df["BASE_CAGR%"].mean()), 3),
            "MeanMaxDD%": round(float(fold_df["BASE_MaxDD%"].mean()), 3),
        },
        {
            "Method": "H8.2HM",
            "MeanSharpe": round(float(fold_df["H8_Sharpe"].mean()), 5),
            "MeanCAGR%": round(float(fold_df["H8_CAGR%"].mean()), 3),
            "MeanMaxDD%": round(float(fold_df["H8_MaxDD%"].mean()), 3),
            "MeanRiskBudget": round(float(fold_df["MeanRiskBudget"].mean()), 4),
            "MeanInterventionRate": round(float(fold_df["InterventionRate"].mean()), 4),
        },
    ])
...
def final_report_text_h8(cfg: Mahoraga8Config, wf: Dict[str, Any]) -> str:
    ...
    so = m6.summarize(h8_r, h8_eq, pd.Series(1.0, index=h8_r.index), pd.Series(0.0, index=h8_r.index), cfg, "H8_2HM_OOS")

    lines = []
    lines.append("MAHORAGA 8.2 HM — FINAL REPORT")
    lines.append("=" * 78)
    lines.append("")
    lines.append("OOS COMPARISON")
    lines.append(f"  BASELINE CAGR={sb['CAGR']*100:.2f}%  Sharpe={sb['Sharpe']:.3f}  MaxDD={sb['MaxDD']*100:.2f}%  CVaR5={sb['CVaR_5']*100:.2f}%")
    lines.append(f"  H8.2HM   CAGR={so['CAGR']*100:.2f}%  Sharpe={so['Sharpe']:.3f}  MaxDD={so['MaxDD']*100:.2f}%  CVaR5={so['CVaR_5']*100:.2f}%")
    lines.append("")
    lines.append("SELECTION AUDIT")
    lines.append(sel_df.to_string(index=False))
    lines.append("")
...
def save_outputs_h8(cfg: Mahoraga8Config, wf: Dict[str, Any]) -> None:
    ...
    build_fold_summary(wf).to_csv(f"{d}/walk_forward_folds_8_2hm.csv", index=False)
