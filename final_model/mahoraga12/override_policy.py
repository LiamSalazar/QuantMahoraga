...
from mahoraga12_utils import iter_grid, sigmoid


def iter_policy_candidates(cfg: Mahoraga12Config):
...
def build_override_weekly(
    ...
    transition_path = pd.Series(
        sigmoid(
            2.0 * (-out["base_ret_1w"])
            + 1.2 * (-out["base_dd_change_2w"])
            + 0.9 * (-out["base_dd_velocity_2w"])
            + 1.0 * out["loss_share_2w"]
            + 1.0 * out["stop_density_2w"]
    ...
    out["structural_score"] = (
        ...
        + 0.24 * out["structural_path_score"]
        + 0.08 * hawkes_weight * stress_hawkes
    ).clip(0.0, 1.0)
    out["transition_score"] = (
        0.60 * out["transition_p"]
        + 0.25 * out["transition_path_score"]
        + 0.15 * hawkes_weight * stress_hawkes
    ).clip(0.0, 1.0)
    out["recovery_score"] = (
        0.58 * out["recovery_p"]
    ...
    for dt in out.index:
        ...
        transition_guard = (
            (row["base_ret_1w"] <= -0.02)
            or (row["base_dd_change_2w"] <= -0.02)
            or (row["base_scale_gap"] >= 0.08)
            or (row["stress_hawkes_norm"] >= 0.55)
        )
        recovery_guard = (
        ...
        if structural_on:
            structural_exit_ok = (
                (row["structural_score"] <= structural_exit and recovery_guard)
        ...
        recovery_admissible = (
            ...
            and (row["structural_score"] <= structural_enter - 0.08)
            and (row["transition_score"] <= transition_enter + 0.08)
        )
        if recovery_admissible:
            out.loc[dt, "override_type"] = "TRANSITION_RECOVERY"
            out.loc[dt, "override_detail"] = "RECOVERY_LIFT"
            out.loc[dt, "defense_blend"] = 0.0
            out.loc[dt, "gate_scale"] = float(policy_params["recovery_gate"])
        ...
        stress_memory = max(0, stress_memory - 1)

    return out


def weekly_to_daily_override(override_weekly: pd.DataFrame, idx: pd.DatetimeIndex, cfg: Mahoraga12Config) -> pd.DataFrame:
    cols = [
        "override_type",
        "override_detail",
        "defense_blend",
