...
def run_mahoraga4_legacy_unused(make_plots_flag: bool = True, run_robustness: bool = True) -> Dict:
    print("="*80)
    print("  LEGACY UNUSED BLOCK — Research Edition")
    print("="*80)
    print(UNIVERSE_BIAS_DISCLAIMER)
...
def save_outputs(
    ...
    if out.get("res") is not None and "corr_scale" in out["res"]:
        ...
        df_corr.to_csv(f"{d}/corr_shield_diagnostics_legacy_unused.csv")
    if rob.get("exec_stress") is not None:
        rob["exec_stress"].to_csv(f"{d}/execution_sensitivity_stress.csv", index=False)
    if rob.get("alt_univ") is not None:
        rob["alt_univ"].to_csv(f"{d}/alt_universes.csv", index=False)
    ...
    if rob.get("stop_ablation") is not None:
        rob["stop_ablation"].to_csv(f"{d}/stop_ablation.csv", index=False)
    if rob.get("ic_ablation") is not None:
        rob["ic_ablation"].to_csv(f"{d}/ic_ablation_legacy_unused.csv", index=False)
...
def _selection_audit_from_sweeps(all_sweeps: pd.DataFrame, last_best: Optional[Dict]) -> Tuple[pd.DataFrame, Dict[str, Dict]]:
    """Build two selection views:
      1) operational = last_fold_winner
      2) audit-stable = avg_val_across_folds
    Returns (audit_df, info_by_source).
    """
    rows = []
    info = {}
    if last_best is not None:
        last_row = {
            'source': 'last_fold_winner',
            'folds_seen': 1,
            'combo_params': deepcopy(last_best.get('combo_params', {})),
            'val_score': float(last_best.get('score', np.nan)),
            'val_sharpe': float(last_best.get('s_val', {}).get('Sharpe', np.nan)),
            'val_p_value': float(last_best.get('p_value', np.nan)),
            'val_q_value': float(last_best.get('q_value', np.nan)),
            'val_sig_5pct': bool(last_best.get('sig_5pct', False)),
            'val_stat_label': str(last_best.get('stat_label', 'unknown')),
            'any_sig_5pct': bool(last_best.get('sig_5pct', False)),
            'frac_sig_5pct': 1.0 if bool(last_best.get('sig_5pct', False)) else 0.0,
            'note': 'Operational selection: best combo on the last validation fold.'
        }
        rows.append(last_row)
        info['last_fold_winner'] = deepcopy(last_row)

    if all_sweeps is not None and not all_sweeps.empty:
        grid_keys = [k for k in SWEEP_GRID.keys() if k in all_sweeps.columns]
        if grid_keys:
            agg = (all_sweeps.groupby(grid_keys, dropna=False)
                   .agg(folds_seen=('fold', 'nunique'),
                        mean_score=('score_val', 'mean'),
                        median_score=('score_val', 'median'),
                        mean_val_sharpe=('VAL_Sharpe', 'mean'),
                        median_p=('p_value', 'median'),
                        median_q=('q_value', 'median'),
                        min_q=('q_value', 'min'),
                        any_sig_5pct=('significant_5pct', 'max'),
                        frac_sig_5pct=('significant_5pct', 'mean'),
                        any_sig_10pct=('significant_10pct', 'max'),
                        frac_sig_10pct=('significant_10pct', 'mean'))
                   .reset_index())
            if not agg.empty:
                agg = agg.sort_values(['mean_score', 'mean_val_sharpe', 'median_q'], ascending=[False, False, True]).reset_index(drop=True)
                best = agg.iloc[0]
                combo = {k: best[k] for k in grid_keys}
                stat_label = (
                    'agg_support_all_folds' if float(best['frac_sig_5pct']) >= 0.999 else
                    'agg_support_partial' if bool(best['any_sig_5pct']) else
                    'agg_mean_score_stat_weak'
                )
                avg_row = {
                    'source': 'avg_val_across_folds',
                    'folds_seen': int(best['folds_seen']),
                    'combo_params': combo,
                    'val_score': float(best['mean_score']),
                    'val_sharpe': float(best['mean_val_sharpe']),
                    'val_p_value': float(best['median_p']),
                    'val_q_value': float(best['median_q']),
                    'val_sig_5pct': float(best['frac_sig_5pct']) >= 0.999,
                    'val_stat_label': stat_label,
                    'any_sig_5pct': bool(best['any_sig_5pct']),
                    'frac_sig_5pct': float(best['frac_sig_5pct']),
                    'note': 'Audit-stable selection: combo with highest mean validation score across folds.'
                }
                rows.append(avg_row)
                info['avg_val_across_folds'] = deepcopy(avg_row)
    audit_df = pd.DataFrame(rows)
    return audit_df, info
...
def run_fold_sweep(
    ...
    """Version 6.1: use threading by default on Windows to avoid pickling/tempfile blowups."""
    keys      = list(SWEEP_GRID.keys())
    combos    = list(iproduct(*[SWEEP_GRID[k] for k in keys]))
    base_seed = cfg_base.random_seed

    use_parallel = bool(cfg_base.parallel_sweep and _JOBLIB_AVAILABLE and len(combos) > 4)
    backend = 'threading' if os.name == 'nt' else 'loky'

    if use_parallel:
        print(f"  [sweep] parallel/{backend}, {len(combos)} combos …")
        n_jobs = max(1, (os.cpu_count() or 2) - 1)
        raw = Parallel(n_jobs=n_jobs, backend=backend, verbose=0)(
            joblib_delayed(_evaluate_single_combo)(
                c, keys, cfg_base, ic_weights, ohlcv, costs,
                val_start, val_end, fold_n, base_seed, universe_schedule
            ) for c in combos
    ...
    best_row = sorted(final, key=lambda x: (-x['score_val'], -x['VAL_Sharpe'], x['q_value']))[0]
    best = {
        'score': best_row['score_val'],
        'cfg': best_row['_cfg'],
        's_val': best_row['_s_val'],
        's_qqq': best_row['_s_qqq'],
        'combo_params': best_row['combo_params'],
        'p_value': best_row['p_value'],
        'q_value': best_row['q_value'],
        'sig_5pct': best_row['significant_5pct'],
        'stat_label': best_row['stat_label'],
    }
    df = df.drop(columns=['_cfg', '_s_val', '_s_qqq'], errors='ignore').sort_values('score_val', ascending=False).reset_index(drop=True)
    return df, best
...
def run_walk_forward(
    ...
    for fold in folds:
        ...
        fold_results.append({
            ...
            **{f'best_{k}': v for k, v in best['combo_params'].items()},
            ...
            'AvgCrisisScale': round(float(res_test['crisis_scale'].loc[r_test.index].mean()), 4),
            'AvgVolScale': round(float(res_test['vol_scale'].loc[r_test.index].mean()), 4),
            'AvgCorrScale': round(float(res_test.get('corr_scale', pd.Series(1.0, index=r_test.index)).loc[r_test.index].mean()), 4),
            'AvgCash%': round(float((1.0 - exp_test).clip(lower=0.0).mean() * 100), 2),
            'ScoreDispersion': round(score_disp, 6) if np.isfinite(score_disp) else np.nan,
        })
        fold_runtime.append({
        ...
        partial_str = ' [PARTIAL FOLD]' if is_partial else ''
        print(f"  [fold {fold_n}] Sharpe={s_test['Sharpe']:.3f}  CAGR={s_test['CAGR']*100:.1f}%  DD={s_test['MaxDD']*100:.1f}%  q={best['q_value']:.4f}  {best['stat_label']}{partial_str}")
    ...
    selection_audit_df, info_by_source = _selection_audit_from_sweeps(all_sweeps_df, last_best)
    ...
    selected_config_info['fold'] = len(fold_results)
    selected_config_info['fold_runtime'] = fold_runtime
    ...
    return oos_r, oos_eq, oos_exp, oos_to, fold_results, all_sweeps_df, oos_label, selected_config_info


def ic_weight_ablation(ohlcv, cfg_win: Mahoraga4Config, costs: CostsConfig,
                       universe_schedule: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    """6.1: real ablation + plumbing diagnostic.
    If IC weights are numerically equal to uniform, report it explicitly instead of pretending there is a difference.
    """
    equal_w = np.array([1/3, 1/3, 1/3], dtype=float)
    ic_w = np.array([float(cfg_win.w_trend), float(cfg_win.w_mom), float(cfg_win.w_rel)], dtype=float)
    same_as_equal = bool(np.allclose(ic_w, equal_w, atol=1e-6))
    modes = [
        ('ablation', 'IC_weights', tuple(ic_w.tolist())),
        ('ablation', 'EQUAL_WEIGHTS', (1/3, 1/3, 1/3)),
        ('plumbing_diag', 'TREND_ONLY', (1.0, 0.0, 0.0)),
        ('plumbing_diag', 'MOM_ONLY', (0.0, 1.0, 0.0)),
        ('plumbing_diag', 'REL_ONLY', (0.0, 0.0, 1.0)),
    ]
    rows = []
    for test_type, mode, (wt, wm, wr) in modes:
        c2 = deepcopy(cfg_win)
        c2.w_trend = float(wt)
        c2.w_mom = float(wm)
        c2.w_rel = float(wr)
        ...
        s = summarize(r, res['equity'], res['exposure'], res['turnover'], c2, mode)
        note = ''
        if mode == 'IC_weights' and same_as_equal:
            note = 'IC weights equal to uniform in this run; no delta vs equal weights should be expected.'
        rows.append({
            'TestType': test_type,
            'Mode': mode,
            'w_trend': round(c2.w_trend, 6),
            'w_mom': round(c2.w_mom, 6),
            'w_rel': round(c2.w_rel, 6),
            'SameAsEqual': bool(np.allclose([c2.w_trend, c2.w_mom, c2.w_rel], equal_w, atol=1e-6)),
            'CAGR%': round(s['CAGR'] * 100, 3),
            'Vol%': round(s['AnnVol'] * 100, 3),
            'Sharpe': round(s['Sharpe'], 4),
            'MaxDD%': round(s['MaxDD'] * 100, 3),
...
def corr_shield_ablation(
    ...
    for mode, flag in [('CORR_SHIELD_ON', True), ('CORR_SHIELD_OFF', False)]:
        ...
        r = res['returns_net']
        s = summarize(r, res['equity'], res['exposure'], res['turnover'], c2, mode)
        corr_scale = to_s(res.get('corr_scale', pd.Series(1.0, index=r.index))).reindex(r.index).fillna(1.0)
        corr_state = to_s(res.get('corr_state', pd.Series('disabled', index=r.index))).reindex(r.index).fillna('disabled')
        out = {
            'Mode': mode,
            'CAGR%': round(s['CAGR'] * 100, 3),
            'Vol%': round(s['AnnVol'] * 100, 3),
            ...
            'AvgExp%': round(s['AvgExposure'] * 100, 2),
            'TurnAnn': round(s['TurnoverAnn'], 3),
            'ShieldActivePct': round(float((corr_scale < 0.999999).mean() * 100), 2),
            'HardKillDays': int((corr_state.astype(str) == 'hard_kill').sum()),
            'MinCorrScale': round(float(corr_scale.min()), 4),
            'MeanCorrScale': round(float(corr_scale.mean()), 4),
        }
        if vix_available:
            vix = to_s(ohlcv['close'][cfg_win.bench_vix].reindex(r.index).ffill(), 'VIX')
            mask = (vix >= vix_thr).reindex(r.index).fillna(False)
            r_p = to_s(r)[mask]
            out['PanicDays'] = int(mask.sum())
            if len(r_p) >= 25:
                eq_p = c2.capital_initial * (1.0 + r_p).cumprod()
                s_p = summarize(r_p, eq_p,
                                res['exposure'].reindex(r_p.index).fillna(0.0),
...
def print_results(out: Dict, fold_results: List[Dict], ff, selected_config_info: Dict):
    sep = '=' * 110
    print(UNIVERSE_BIAS_DISCLAIMER)
    print(f"\n{sep}\n  MAHORAGA 6.1 — FULL RESULTS\n{sep}")
    oos_label = out.get('oos_label', 'OOS')

    print(f"\n{'─'*70}  FULL PERIOD")
    print(pd.DataFrame([_fmt(out['full']), _fmt(out['qqq_full']), _fmt(out['eqw_full']), _fmt(out['mom_full'])]).to_string(index=False))

    print(f"\n{'─'*70}  OOS — {oos_label}")
    print(pd.DataFrame([_fmt(out['oos']), _fmt(out['qqq_oos']), _fmt(out['eqw_oos']), _fmt(out['mom_oos'])]).to_string(index=False))
    ...
    print(f"  sig@5%FDR:   {selected_config_info['val_sig_5pct']}")
    print(f"  stat_label:  {selected_config_info['val_stat_label']}")
    if 'any_sig_5pct' in selected_config_info:
        print(f"  any_sig_5pct_across_support: {selected_config_info.get('any_sig_5pct')}")
        print(f"  frac_sig_5pct_across_support:{selected_config_info.get('frac_sig_5pct', np.nan):.3f}")
    if not selected_config_info.get('val_sig_5pct'):
        print('  [NOTE] Config does NOT pass BHY@5% as selected support — interpret with caution.')

    audit_df = selected_config_info.get('selection_audit_df')
    if isinstance(audit_df, pd.DataFrame) and not audit_df.empty and len(audit_df) > 1:
        print(f"\n{'─'*70}  SELECTION AUDIT (operational vs stable)")
        audit_show = audit_df.copy()
        audit_show['combo_params'] = audit_show['combo_params'].astype(str)
        print(audit_show.to_string(index=False))
...
def _build_final_report_text(out: Dict, fold_results: List[Dict], selected_config_info: Dict, oos_label: str) -> str:
    cfg = out.get('cfg')
    lines = [
        'MAHORAGA 6.1 — FINAL REPORT',
        '=' * 80,
        UNIVERSE_BIAS_DISCLAIMER,
        '',
        f'OOS type: {oos_label}',
    ...
    if cfg is not None:
        lines += ['', 'CORRELATION SHIELD (v6.1)', '-' * 40]
        lines.append(f"  enabled: {bool(getattr(cfg, 'enable_corr_shield', False))}")
        lines.append(f"  corr_window: {getattr(cfg, 'corr_window', '—')}")
        lines.append(f"  rho_in / rho_out: {getattr(cfg, 'corr_rho_in', '—')} / {getattr(cfg, 'corr_rho_out', '—')}")
        lines.append(f"  gamma: {getattr(cfg, 'corr_gamma', '—')}")
    ...
    if isinstance(audit_df, pd.DataFrame) and not audit_df.empty:
        lines += ['', 'SELECTION AUDIT', '-' * 40]
        for _, row in audit_df.iterrows():
            lines.append(f"  [{row['source']}] combo={row['combo_params']}  score={row['val_score']:.4f}  q={row['val_q_value']:.6f}  sig5={row['val_sig_5pct']}")
...
def save_outputs(
    ...
    meta = {
        ...
        'data_quality_layer': cfg.enable_data_quality_layer,
        'ml_regime_gate': bool(getattr(cfg, 'enable_ml_regime_gate', False)),
        'final_selection_method': getattr(cfg, 'final_selection_method', 'last_fold_winner'),
    }
    pd.DataFrame([meta]).to_csv(f'{d}/walk_forward_meta.csv', index=False)
    ...
    support = {
        **selected_config_info.get('combo_params', {}),
        ...
        'val_stat_label': selected_config_info['val_stat_label'],
        'val_sharpe': selected_config_info.get('val_sharpe', np.nan),
        'any_sig_5pct': selected_config_info.get('any_sig_5pct', np.nan),
        'frac_sig_5pct': selected_config_info.get('frac_sig_5pct', np.nan),
    }
    pd.DataFrame([support]).to_csv(f'{d}/selected_config_support.csv', index=False)
    audit_df = selected_config_info.get('selection_audit_df')
    if isinstance(audit_df, pd.DataFrame) and not audit_df.empty:
        audit_save = audit_df.copy()
        audit_save['combo_params'] = audit_save['combo_params'].astype(str)
        audit_save.to_csv(f'{d}/selection_audit.csv', index=False)
    ...
    all_sweeps.to_csv(f'{d}/walk_forward_sweeps.csv', index=False)
    out['stress'].to_csv(f'{d}/stress_full.csv', index=False)
    out['stress_oos'].to_csv(f'{d}/stress_oos.csv', index=False)
    out['regime_full'].to_csv(f'{d}/regime_full.csv', index=False)
    out['regime_oos'].to_csv(f'{d}/regime_oos.csv', index=False)
    ic_df.to_csv(f'{d}/rolling_ic_multi.csv', index=False)

    if out.get('res') is not None and 'corr_scale' in out['res']:
        df_corr = pd.DataFrame({
            'date': out['res']['equity'].index,
            'corr_rho': to_s(out['res'].get('corr_rho')).reindex(out['res']['equity'].index).values,
            'corr_scale': to_s(out['res'].get('corr_scale')).reindex(out['res']['equity'].index).values,
            'corr_state': to_s(out['res'].get('corr_state')).reindex(out['res']['equity'].index).astype(str).values,
        })
        df_corr.to_csv(f'{d}/corr_shield_diagnostics.csv', index=False)
    if rob.get('exec_stress') is not None:
        rob['exec_stress'].to_csv(f'{d}/execution_sensitivity_stress.csv', index=False)
    if rob.get('alt_univ') is not None:
        rob['alt_univ'].to_csv(f'{d}/alt_universes.csv', index=False)
    ...
    print(f"\n  [outputs → ./{d}/]")
    print('    comparison_full.csv, comparison_oos.csv, walk_forward_folds.csv, fold_diagnostics.csv')
    print('    walk_forward_meta.csv, selected_config_support.csv, selection_audit.csv, final_report.txt, walk_forward_sweeps.csv')
    print('    corr_shield_diagnostics.csv, corr_shield_ablation.csv, ic_ablation.csv, stress_oos.csv')


def run_mahoraga6(make_plots_flag: bool = True, run_robustness: bool = True) -> Dict:
    print('=' * 80)
    print('  MAHORAGA 6.1 — Research Edition')
    print('=' * 80)
    print(UNIVERSE_BIAS_DISCLAIMER)

    cfg = Mahoraga6Config()
    cfg.outputs_dir = 'mahoraga6_1_outputs'
    cfg.plots_dir = 'mahoraga6_1_plots'
    cfg.label = 'MAHORAGA_6_1'
    costs = CostsConfig()
    ucfg = UniverseConfig()
    _ensure_dir(cfg.cache_dir)
    _ensure_dir(cfg.plots_dir)
    ...
    print('\n[2] Walk-forward …')
    oos_r, oos_eq, oos_exp, oos_to, fold_results, all_sweeps, oos_label, selected_config_info = run_walk_forward(
        ohlcv, cfg, costs, universe_schedule=universe_schedule
    )
    oos_summary = summarize(oos_r, oos_eq, oos_exp, oos_to, cfg, oos_label)
    print(f"\n  OOS type:   {oos_label}")
    print(f"  OOS Sharpe={oos_summary['Sharpe']:.3f}  CAGR={oos_summary['CAGR']*100:.1f}%  MaxDD={oos_summary['MaxDD']*100:.1f}%")

    print('\n[3] Applying selected final combo as final config …')
    last_train_end = cfg.wf_folds[-1][0]
    cfg_final = deepcopy(cfg)
    qqq_full = to_s(ohlcv['close'][cfg.bench_qqq].ffill())
    dd_thr, vol_thr = calibrate_crisis_thresholds(qqq_full, cfg.wf_train_start, last_train_end, cfg_final)
    ...
    print(f"\n  Final config source: {selected_config_info['source']}")
    print(f"  Final config: {selected_config_info['combo_params']}")
    print(f"  Statistical support: p={selected_config_info['val_p_value']:.6f}  q={selected_config_info['val_q_value']:.6f}  sig@5%={selected_config_info['val_sig_5pct']}  label={selected_config_info['val_stat_label']}")
    ...
    if run_robustness:
        ...
        rob['ic_ablation'] = ic_weight_ablation(ohlcv, cfg_final, costs, universe_schedule=universe_schedule)
        print('\n[7f] Correlation Shield ablation …')
        rob['corr_ablation'] = corr_shield_ablation(ohlcv, cfg_final, costs, universe_schedule=universe_schedule)
